...
BEGIN
  ...
  -- Pick the POS-sale debit account that is a child of 2180 (any employee sub-account)
  ...
  WHERE t.user_id = NEW.user_id
    ...
    AND t.reference = NEW.source_reference
    AND a.parent_code = '2180'
  ORDER BY t.created_at ASC
  LIMIT 1;

  IF v_src_debit IS NULL THEN
    RAISE WARNING 'auto_journal_pos_meal: source POS tx (employee acc) not found for ref %', NEW.source_reference;
    RETURN NEW;
  END IF;
