...
DROP FUNCTION IF EXISTS public.hr_backfill_attendance(uuid, date, date, time, time, text);
...
BEGIN
  ...
  WHILE v_day <= p_to LOOP
    BEGIN
      ...
      ELSE
        -- Overwrite mode: clear existing events for this day (kept in audit log below)
        ...
        INSERT INTO public.attendance_events(
          ...
          p_employee_id, v_emp.user_id, v_emp.branch_id, 'check_in', v_check_in_ts, 'manual', p_reason, true
        ) RETURNING id INTO v_event_in_id;

        INSERT INTO public.attendance_events(
          employee_id, auth_user_id, branch_id, event_type, event_time, status, notes, server_recorded
        ) VALUES (
          p_employee_id, v_emp.user_id, v_emp.branch_id, 'check_out', v_check_out_ts, 'manual', p_reason, true
...
REVOKE ALL ON FUNCTION public.hr_backfill_attendance(uuid, date, date, time, time, text, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.hr_backfill_attendance(uuid, date, date, time, time, text, boolean) TO authenticated;
