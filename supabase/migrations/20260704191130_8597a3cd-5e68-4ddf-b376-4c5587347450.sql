-- Fix: attendance_breaks has no `status` column, but the shared
-- enforce_attendance_tenant_match() trigger references NEW.status.
-- Guard the status check so it only runs when the row actually has it.
