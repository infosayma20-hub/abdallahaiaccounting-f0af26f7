...
BEGIN
  ...
  CASE p_event
    ...
    WHEN 'endorse' THEN
      ...
      IF p_endorsed_to_contact_id IS NOT NULL THEN v_contact_id := p_endorsed_to_contact_id; END IF;
    WHEN 'cancel' THEN
  ...
  SET status = v_new_status::cheque_status, linked_transaction_id = COALESCE(linked_transaction_id, v_tx_id), updated_at = now()
  WHERE id = p_cheque_id;

  -- FIX: use 'details' jsonb column (not non-existent 'notes')
  BEGIN
    INSERT INTO public.cheque_status_history(cheque_id, user_id, from_status, to_status, action_type, reason, linked_transaction_id, details)
