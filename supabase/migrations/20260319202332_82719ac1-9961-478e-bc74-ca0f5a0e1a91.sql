...
BEGIN
  ...
  -- Sale entry: one single GL entry for the sale amount
  -- Change (باقي) is NOT recorded as a separate GL entry.
  -- It is tracked in pos_payments.change_amount and reflected in session reconciliation.
  -- This prevents the change from being incorrectly classified as an expense.
