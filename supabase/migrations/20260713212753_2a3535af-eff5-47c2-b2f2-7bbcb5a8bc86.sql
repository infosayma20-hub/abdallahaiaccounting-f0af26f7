...
BEGIN
  ...
  classified AS (
    SELECT ds.*,
      -- Prefer work_shifts start (real shift). Fall back to actual check-in hour
      -- (employees.shift_start defaults to 08:00 for everyone → not reliable).
      COALESCE(
  ...
  SELECT jsonb_build_object(
    ...
    'meta', jsonb_build_object(
      ...
      'split_method', 'work_shifts start-time, else actual first check-in hour. Morning<12h, Mid 12–15h, Evening>=15h. Mid split at 17:00 local.',
