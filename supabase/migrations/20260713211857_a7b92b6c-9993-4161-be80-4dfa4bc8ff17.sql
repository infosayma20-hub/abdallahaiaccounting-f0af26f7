...
BEGIN
  ...
  day_source AS (
    SELECT
      ...
      e.shift_start,
      e.shift_end,
      sl.name AS shift_name,
      sl.start_time AS ws_start_time,
      sl.end_time AS ws_end_time,
  ...
  day_split_raw AS (
    SELECT
      ds.*,
      CASE
        WHEN ds.local_in IS NOT NULL AND ds.local_out IS NOT NULL AND ds.local_out > ds.local_in THEN
          GREATEST(0, EXTRACT(EPOCH FROM (LEAST(ds.local_out, ds.day_start + interval '17 hours') - GREATEST(ds.local_in, ds.day_start + interval '9 hours'))) / 3600.0)
        ELSE 0
      END AS raw_day_hours,
      CASE
        WHEN ds.local_in IS NOT NULL AND ds.local_out IS NOT NULL AND ds.local_out > ds.local_in THEN
          GREATEST(0, EXTRACT(EPOCH FROM (LEAST(ds.local_out, ds.day_start + interval '30 hours') - GREATEST(ds.local_in, ds.day_start + interval '17 hours'))) / 3600.0)
        ELSE 0
      END AS raw_evening_hours
    FROM day_source ds
  ),
  detail_rows AS (
    SELECT
      ...
      CASE
        WHEN (r.raw_day_hours + r.raw_evening_hours) > 0.01 AND r.net_hours > 0 THEN r.raw_day_hours * r.net_hours / (r.raw_day_hours + r.raw_evening_hours)
        WHEN r.net_hours > 0 THEN r.net_hours
        ELSE 0
      END AS day_hours,
      CASE
        WHEN (r.raw_day_hours + r.raw_evening_hours) > 0.01 AND r.net_hours > 0 THEN r.raw_evening_hours * r.net_hours / (r.raw_day_hours + r.raw_evening_hours)
        ELSE 0
  ...
  sales_agg AS (
    SELECT branch_id, date, sum(total)::numeric AS sales_total
    FROM sales_orders
  ...
  sales_hourly AS (
    ...
    FROM sales_orders
    GROUP BY branch_id, date, sale_hour
  ),
  sales_hourly_json AS (
    ...
    LEFT JOIN sales_hourly sh
     ...
     AND sh.date = keys.date
     AND sh.sale_hour = h.hr
  ...
  dept_agg AS (
    SELECT
      ...
      sum(dr.overtime_hours)::numeric AS overtime_hours
    FROM detail_rows dr
    GROUP BY dr.branch_id, dr.date, dr.department
  ),
  dept_json AS (
    SELECT
      da.branch_id,
    ...
    FROM dept_agg da
    GROUP BY da.branch_id, da.date
  ),
  all_keys AS (
    SELECT branch_id, date FROM attendance_agg
    UNION
    SELECT branch_id, date FROM sales_agg
  ),
  rows_json AS (
  ...
  SELECT jsonb_build_object(
    ...
    'meta', jsonb_build_object(
      ...
      'split_method', 'attendance_days first/last check-in split by local business day',
  ...
  FROM rows_json r
  CROSS JOIN details_json d
  CROSS JOIN branches_json b;
...
CREATE INDEX IF NOT EXISTS idx_attendance_days_auth_user_date_branch
  ON public.attendance_days (auth_user_id, attendance_date, branch_id);

CREATE INDEX IF NOT EXISTS idx_correction_requests_auth_status_date_employee
  ON public.correction_requests (auth_user_id, status, attendance_date, employee_id);

CREATE INDEX IF NOT EXISTS idx_pos_orders_user_paid_business_session
  ON public.pos_orders (user_id, business_date, session_id)
  WHERE state = 'paid';

CREATE INDEX IF NOT EXISTS idx_pos_sessions_id_terminal
  ON public.pos_sessions (id, terminal_id);

CREATE INDEX IF NOT EXISTS idx_pos_terminals_user_branch
  ON public.pos_terminals (user_id, branch_id, id);

CREATE INDEX IF NOT EXISTS idx_transactions_deleted_lookup
  ON public.transactions (id, is_deleted);