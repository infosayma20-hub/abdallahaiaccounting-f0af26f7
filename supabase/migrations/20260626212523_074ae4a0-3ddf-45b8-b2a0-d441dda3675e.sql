...
BEGIN
  ...
  IF p_new_currency IS NOT NULL AND p_new_method <> 'mixed' THEN
    IF upper(p_new_currency) = 'ILS' THEN
      ...
      UPDATE pos_payments SET currency='ILS', exchange_rate=1,
       ...
       WHERE order_id = p_order_id;
      -- 🔑 sync header for shift-close calculation
      UPDATE pos_orders
  ...
  -- 🔑 Mixed payment → reset header currency to ILS (split only supports ILS)
  IF p_new_method = 'mixed' THEN
    UPDATE pos_orders
       SET payment_currency='ILS',
  ...
  -- 🔑 Switching to/from employee_account or other single methods without
  --     currency change → header currency stays as-is (already ILS for these
  --     methods in 99% of cases). Force-normalize to ILS when switching to
  --     employee_account since it's always ILS by design.
  IF p_new_method = 'employee_account' THEN
    UPDATE pos_orders
       SET payment_currency='ILS',
           payment_currency_amount=COALESCE(v_order.total, 0),
  ...
  RETURN jsonb_build_object('ok', true, 'order_id', v_order.id,
    ...
    'employee_id', p_employee_id, 'header_synced', true);
END;
$function$;