...
-- 1. Drop the problematic triggers that conflict with handle_new_user
DROP TRIGGER IF EXISTS trg_auto_assign_admin ON public.profiles;
DROP TRIGGER IF EXISTS on_profile_created_assign_trial ON public.profiles;

-- 2. Update handle_new_user to also link employee to admin's company
