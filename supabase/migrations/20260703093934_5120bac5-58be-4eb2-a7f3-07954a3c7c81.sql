...
AS $$
  WITH parents AS (
    ...
    WHERE account_code IN ('2110','2111','1130','1131')
      AND user_id = public.get_current_tenant_owner_id()
  ),
  base AS (
    SELECT t.*,
      CASE
    ...
    WHERE t.is_deleted = false
      AND t.user_id = public.get_current_tenant_owner_id()
      AND (
  ...
  SELECT
    ...
    (SELECT a.account_name FROM public.accounts a WHERE a.account_code =
       CASE WHEN b.side_ = 'debit' THEN b.credit_account_code ELSE b.debit_account_code END
       AND a.user_id = public.get_current_tenant_owner_id() LIMIT 1),
    b.contact_id,
...
GRANT EXECUTE ON FUNCTION public.list_parent_account_transactions(boolean) TO authenticated;
...
AS $$
  ...
  WHERE a.user_id = public.get_current_tenant_owner_id()
...
GRANT EXECUTE ON FUNCTION public.search_leaf_accounts(text, text, int) TO authenticated;
...
AS $$
DECLARE
  v_tenant uuid := public.get_current_tenant_owner_id();
...
BEGIN
  ...
  VALUES
    ('manual_parent_reroute', 'transaction', p_transaction_id,
     jsonb_build_object('side', v_side, 'account_code', v_old_code, 'contact_id', v_txn.contact_id),
     jsonb_build_object('side', v_side, 'account_code', v_new_acc.account_code, 'contact_id', COALESCE(p_new_contact_id, v_txn.contact_id)),
     'إعادة توجيه يدوية من حساب أب إلى حساب فرعي',
     auth.uid());

  RETURN jsonb_build_object('success', true, 'transaction_id', p_transaction_id, 'new_code', v_new_acc.account_code);
END;
$$;

GRANT EXECUTE ON FUNCTION public.reroute_parent_transaction(uuid, text, uuid) TO authenticated;
