...
-- Add user_id to malaki_portal_users to link members to the account owner
ALTER TABLE public.malaki_portal_users
ADD COLUMN IF NOT EXISTS user_id UUID;

-- Update malaki_create_user to accept user_id and use email as login
...
DECLARE v_id UUID;
BEGIN
  INSERT INTO public.malaki_portal_users (username, password_hash, full_name, role, can_see_sales, can_see_liquidity, can_see_all_branches, user_id)
  VALUES (lower(trim(p_username)), crypt(p_password, gen_salt('bf')), p_full_name, p_role, p_can_see_sales, p_can_see_liquidity, p_can_see_all_branches, p_user_id)
  RETURNING id INTO v_id;
  RETURN jsonb_build_object('success', true, 'id', v_id);
EXCEPTION WHEN unique_violation THEN
  RETURN jsonb_build_object('success', false, 'error', 'البريد الإلكتروني أو اسم المستخدم موجود مسبقاً');
END;
$$;

-- Update verify_malaki_login to support email login
...
BEGIN
  ...
  WHERE (username = lower(trim(p_username)) OR email = lower(trim(p_username))) AND is_active = true;
