DROP FUNCTION public.create_mixed_voucher_atomic(uuid, text, uuid, text, date, text, numeric, text, text, text, numeric, text, jsonb, jsonb, text, uuid, uuid, uuid, text);
...
CREATE OR REPLACE FUNCTION public.create_mixed_voucher_atomic(
  ...
  p_counter_account_code text DEFAULT NULL::text,
  p_endorsed_cheque_ids uuid[] DEFAULT NULL::uuid[]
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_endorsed_total numeric := 0;
  v_endorsed_found int := 0;
  v_total numeric;
  v_cheque_tx_id uuid;
  v_cash_tx_id uuid;
  v_endorsed_tx_id uuid;
  v_first_endorsed_tx_id uuid;
...
BEGIN
  ...
  -- Endorsement (تجيير شيكات واردة نملكها) is a payment-side concept only.
  IF p_kind = 'receipt' AND p_endorsed_cheque_ids IS NOT NULL AND array_length(p_endorsed_cheque_ids, 1) > 0 THEN
    RETURN jsonb_build_object('success', false, 'error', 'endorsed cheques are only supported for payment vouchers');
  END IF;
  ...
  SELECT id INTO v_existing FROM public.transactions
    WHERE user_id = p_user_id
      AND (idempotency_key = v_idem
           OR idempotency_key = v_idem || '-CASH'
           OR idempotency_key = v_idem || '-CHQ'
           OR idempotency_key LIKE v_idem || '-END-%')
    LIMIT 1;
  ...
  -- Endorsed cheques: existing incoming cheques we hold, endorsed to the payee.
  IF p_endorsed_cheque_ids IS NOT NULL AND array_length(p_endorsed_cheque_ids, 1) > 0 THEN
    SELECT COALESCE(SUM(amount), 0), COUNT(*)
      INTO v_endorsed_total, v_endorsed_found
      FROM public.cheques
     WHERE id = ANY(p_endorsed_cheque_ids)
       AND user_id = p_user_id
       AND cheque_type = 'وارد'::public.cheque_type
       AND status IN ('مسجل'::public.cheque_status, 'آجل'::public.cheque_status, 'مستحق'::public.cheque_status);
    IF v_endorsed_found <> array_length(p_endorsed_cheque_ids, 1) THEN
      RETURN jsonb_build_object('success', false, 'error', 'بعض الشيكات المُجيّرة غير صالحة أو مظهَّرة مسبقاً');
    END IF;
  END IF;

  v_total := p_cash_amount + v_cheques_total + v_endorsed_total;
  IF v_total <= 0 THEN
  ...
  END IF;
  IF v_endorsed_total > 0 THEN
    PERFORM public._fc_validate_postable_account(p_user_id, '1150');
  END IF;
  ...
  -- Endorsed cheques: per-cheque accounting entry + status lifecycle.
  -- Payment: Dr counter account (supplier/expense) / Cr incoming-cheques drawer.
  IF v_endorsed_total > 0 THEN
    FOR v_end IN
      SELECT id, cheque_number, amount, status, linked_account
        FROM public.cheques
       WHERE id = ANY(p_endorsed_cheque_ids) AND user_id = p_user_id
    LOOP
      INSERT INTO public.transactions(
        user_id, transaction_date, description,
        debit_account_code, credit_account_code, amount, currency,
        ...
        p_user_id, v_date,
        'تجيير شيك رقم ' || COALESCE(v_end.cheque_number,'-') || ' إلى ' || COALESCE(p_contact_name,''),
        v_contact_acct,
        COALESCE(NULLIF(v_end.linked_account,''), '1150'),
        v_end.amount, COALESCE(p_currency,'شيكل'),
        v_tx_type, v_ref, v_idem || '-END-' || v_end.id::text,
        p_contact_id, 'شيك', p_notes,
        CASE WHEN v_is_foreign AND v_use_rate THEN p_exchange_rate ELSE NULL END,
        CASE WHEN v_is_foreign AND v_use_rate THEN ROUND(v_end.amount / p_exchange_rate, 4) ELSE NULL END,
        p_workshop_id, p_cost_center_id,
        v_primary_tx_id
      ) RETURNING id INTO v_endorsed_tx_id;

      v_first_endorsed_tx_id := COALESCE(v_first_endorsed_tx_id, v_endorsed_tx_id);

      UPDATE public.cheques SET
        status = 'مظهر'::public.cheque_status,
        endorsed_to_contact_id = p_contact_id,
        endorsed_to_name = COALESCE(p_contact_name,''),
        endorsed_at = now(),
        endorsement_voucher_id = v_voucher_id
      WHERE id = v_end.id AND user_id = p_user_id;

      INSERT INTO public.cheque_status_history(cheque_id, user_id, from_status, to_status, action_type, reason)
      VALUES (
        v_end.id, p_user_id,
        v_end.status, 'مظهر'::public.cheque_status, 'endorsement',
        'تجيير إلى ' || COALESCE(p_contact_name,'') || ' - سند ' || v_ref
      );
    END LOOP;

    -- Voucher primary link when the voucher is endorsement-only (no cash/new cheques).
    IF v_primary_tx_id IS NULL THEN
      v_primary_tx_id := v_first_endorsed_tx_id;
      UPDATE public.vouchers SET linked_transaction_id = v_primary_tx_id WHERE id = v_voucher_id;
    END IF;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'endorsed_amount', v_endorsed_total,
...
GRANT EXECUTE ON FUNCTION public.create_mixed_voucher_atomic(uuid, text, uuid, text, date, text, numeric, text, text, text, numeric, text, jsonb, jsonb, text, uuid, uuid, uuid, text, uuid[]) TO anon;
