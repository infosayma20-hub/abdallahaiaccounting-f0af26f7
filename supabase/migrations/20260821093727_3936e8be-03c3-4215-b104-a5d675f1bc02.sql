-- 1) خصائص الجلسات: مدفوعة / تُحتسب ضمن السقف
...
UPDATE public.attendance_breaks
   SET is_paid = true, counts_toward_cap = false
 WHERE break_type = 'external_task'
   AND (is_paid = false OR counts_toward_cap = true);

CREATE OR REPLACE FUNCTION public.set_attendance_break_flags()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public
AS $$
BEGIN
  IF NEW.break_type = 'external_task' THEN
    NEW.is_paid := true;
...
DROP TRIGGER IF EXISTS trg_set_attendance_break_flags ON public.attendance_breaks;
CREATE TRIGGER trg_set_attendance_break_flags
BEFORE INSERT OR UPDATE OF break_type ON public.attendance_breaks
FOR EACH ROW EXECUTE FUNCTION public.set_attendance_break_flags();

-- 2) إعادة بناء دالة احتساب اليوم
...
DECLARE
  ...
  v_net_cap_minutes integer := 960;   -- سقف صافي العمل (16 ساعة) للبصمات التلقائية
  v_hard_cap_minutes integer := 1440; -- حد أمان مطلق (24 ساعة)
  ...
  v_break_minutes integer := 0;      -- المخصومة (غير المدفوعة)
  v_all_break_minutes integer := 0;  -- كل الجلسات (للعرض)
...
BEGIN
  ...
  IF v_manual THEN
    -- التعديل اليدوي من الموارد البشرية = مصدر الحقيقة، بلا سقف 16 ساعة
    ...
    ELSE
      ...
      SELECT
        COALESCE(SUM(CASE WHEN b.is_paid THEN 0
  ...
  ELSE
    ...
    -- السقف يُطبَّق على صافي العمل بعد خصم المغادرات (وليس على المدة الإجمالية)
    v_net_minutes := LEAST(GREATEST(0, v_total_minutes - v_break_minutes), v_net_cap_minutes);
...
-- 3) إعادة احتساب الأيام المتأثرة (يدوية أو طويلة) منذ 01/08/2026
DO $$
DECLARE r record;
BEGIN
  FOR r IN
    SELECT ad.employee_id, ad.attendance_date
      FROM public.attendance_days ad
     WHERE ad.attendance_date >= DATE '2026-08-01'
       AND (
         COALESCE(ad.is_manually_adjusted, false)
         OR COALESCE(ad.net_work_minutes, 0) >= 900
         OR EXISTS (
           SELECT 1 FROM public.attendance_breaks b
            WHERE b.attendance_day_id = ad.id AND b.break_type = 'external_task'
         )
       )
  LOOP
    BEGIN
