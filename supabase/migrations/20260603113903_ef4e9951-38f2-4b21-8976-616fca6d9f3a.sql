...
BEGIN
  ...
  WITH agg AS (
    SELECT
      ...
      (array_agg(o.order_number       ORDER BY o.created_at DESC))[1] AS last_order_number_v,
...
-- Drop old signature so the new one is the only callable version.
DROP FUNCTION IF EXISTS public.feedback_followup_queue(date, date, int);

GRANT EXECUTE ON FUNCTION public.feedback_followup_queue(
  date, date, int, text, uuid, text, boolean, text, int, int, int
) TO authenticated;