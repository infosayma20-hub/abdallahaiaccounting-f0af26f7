CREATE OR REPLACE FUNCTION public.hr_update_attendance_day(p_day_id uuid, p_first_check_in timestamp with time zone, p_last_check_out timestamp with time zone, p_status text, p_notes text, p_reason text, p_breaks jsonb DEFAULT '[]'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF p_first_check_in IS NOT NULL AND p_last_check_out IS NULL THEN
    -- دخول بدون خروج: يوم غير مكتمل، الساعات = 0، بدون جلسات
    v_status := 'incomplete';
...
BEGIN
  ...
  IF v_manual THEN
    ...
    -- تزامن: يوم معدّل يدوياً بدخول بدون خروج — إذا وُجدت بصمة خروج حقيقية
    -- لاحقاً نعتمدها تلقائياً ونحسب الساعات ونحدّث الحالة.
    IF v_first IS NOT NULL AND v_last IS NULL THEN
      ...
      IF v_derived_out IS NOT NULL THEN
        ...
        IF v_status = 'incomplete' THEN
          v_status := CASE
            WHEN EXTRACT(HOUR FROM (v_first AT TIME ZONE 'Asia/Hebron')) >= 9 THEN 'late'
            ELSE 'present' END;
        END IF;
      END IF;
    END IF;
