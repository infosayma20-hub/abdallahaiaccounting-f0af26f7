CREATE OR REPLACE FUNCTION public.create_rep_sale_atomic(
  ...
  p_items JSONB, p_idempotency_key TEXT, p_invoice_number TEXT DEFAULT NULL
...
DECLARE
  ...
  v_inv_rpc JSONB; v_recv_rpc JSONB; v_dec_result JSONB;
...
BEGIN
  ...
  IF p_payment_method = 'cash' THEN
    v_recv_rpc := public.create_receipt_with_entry(
      ...
      p_idempotency_key => p_idempotency_key || '-RCV', p_voucher_date => CURRENT_DATE,
      p_exchange_rate => NULL, p_reference => v_invoice_no,
      p_cash_account_code => NULL, p_contact_account_code => NULL,
      p_notes => 'Auto receipt from REP cash sale', p_employee_id => NULL,
      p_workshop_id => NULL, p_allocations => NULL
    );
  END IF;
  ...
  RETURN jsonb_build_object('success', true, 'invoice_id', v_invoice_id, 'invoice_number', v_invoice_no,
    ...
    'has_unknown_cost', v_has_unknown_cost, 'invoice_rpc', v_inv_rpc, 'receipt_rpc', v_recv_rpc);
