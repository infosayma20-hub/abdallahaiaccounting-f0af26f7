...
unioned AS (
  SELECT product_id, name, qty, revenue, cost, 'pos'::text AS src FROM pos_agg
  UNION ALL
  SELECT product_id, name, qty, revenue, cost, 'invoice'::text FROM inv_agg
),
merged AS (
  SELECT
...
GRANT EXECUTE ON FUNCTION public.get_product_profitability(date, date) TO authenticated;