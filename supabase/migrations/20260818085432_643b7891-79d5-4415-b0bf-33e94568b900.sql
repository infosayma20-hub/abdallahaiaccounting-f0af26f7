...
DECLARE
  ...
  v_end timestamptz := ((p_date + 1)::text || ' 12:00:00')::timestamp AT TIME ZONE 'Asia/Hebron';
  v_auth uuid;
  v_branch uuid;
...
BEGIN
  ...
  ELSIF v_first IS NOT NULL THEN
    IF EXTRACT(HOUR FROM (v_first AT TIME ZONE 'Asia/Hebron')) >= 9 THEN
      v_status := 'late';
    END IF;
  END IF;
  ...
  -- Session durations already exclude gaps between a checkout and the next checkin.
  -- Subtract only explicit non-derived breaks recorded inside a working session.
  SELECT GREATEST(0, v_total_minutes - COALESCE(SUM(
    CASE
      WHEN b.reason = 'محسوبة تلقائياً من البصمات' THEN 0
      ELSE GREATEST(0, b.duration_minutes)
    END
  ), 0)::integer)
    INTO v_net_minutes
    FROM public.attendance_breaks b
...
BEGIN
  ...
  IF v_employee_id IS NULL OR v_date IS NULL THEN
    RETURN;
  END IF;

  PERFORM public.recompute_attendance_day(v_employee_id, v_date);
END;
$function$;