CREATE OR REPLACE FUNCTION public.sync_cash_invoice_voucher(p_user_id uuid, p_invoice_id uuid, p_is_cash boolean, p_invoice_type text, p_contact_id uuid, p_contact_name text, p_amount numeric, p_date date, p_cash_account_code text, p_currency text DEFAULT 'شيكل'::text, p_exchange_rate numeric DEFAULT NULL::numeric, p_reference text DEFAULT NULL::text, p_workshop_id uuid DEFAULT NULL::uuid, p_cost_center_id uuid DEFAULT NULL::uuid, p_posted_by uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF NOT COALESCE(p_is_cash, false) OR COALESCE(p_amount,0) <= 0 THEN
    IF v_tx IS NOT NULL THEN
      ...
      UPDATE public.transactions
       ...
       WHERE id = v_tx;
    END IF;
    -- إعادة احتساب حالة الدفع من روابط السندات الفعلية — يمنع مسح paid_amount
    -- بالخطأ عند تعديل فاتورة آجلة عليها سندات يدوية سابقة.
    PERFORM public.recalc_invoice_payment_status(p_invoice_id);
  ...
  ELSE
    ...
    ELSE
      UPDATE public.vouchers SET
        ...
        workshop_id = p_workshopId, cost_center_id = p_cost_center_id
      WHERE id = v_doc_id
  ...
  -- ── تخصيص ذكي: نخصّم من الفاتورة بحد أقصى قيمتها الإجمالية ──
  -- دفع أقل من الإجمالي → المتبقي يبقى ذمة (حالة partial تلقائياً من recalc)
  -- دفع أكثر من الإجمالي → الزيادة تبقى رصيداً غير مخصص على حساب الجهة
  --   (دائن للزبون / سلفة للمورد) — نفس سلوك السندات اليدوية تماماً.
  SELECT COALESCE(total_amount, 0) INTO v_inv_total
  FROM public.invoices WHERE id = p_invoice_id;
  v_alloc_amt := LEAST(p_amount, COALESCE(v_inv_total, 0));

  IF v_alloc_amt > 0.001 THEN
    v_alloc := public.allocate_voucher_to_invoices_atomic(
      p_user_id, NULL, v_tx, p_amount,
      jsonb_build_array(jsonb_build_object('invoice_id', p_invoice_id, 'amount', v_alloc_amt)),
      false
    );
