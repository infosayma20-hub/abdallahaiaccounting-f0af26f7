...
BEGIN
  ...
  -- Authoritative span rule for EVERY employee and EVERY day:
  -- net = (last_check_out - first_check_in) - all recorded departures/breaks.
  -- Multiple sessions and missing/late punches inside the span must not
  -- carve out unpaid holes; only actual recorded departures are deducted.
...
BEGIN
  FOR d IN
    SELECT employee_id, attendance_date
     ...
     WHERE attendance_date >= (CURRENT_DATE - 60)
     ORDER BY attendance_date
  LOOP
    BEGIN
      PERFORM public.recompute_attendance_day(d.employee_id, d.attendance_date);
    EXCEPTION WHEN OTHERS THEN
      NULL; -- skip days blocked by branch/tenant guards
    END;
  END LOOP;
END $$;