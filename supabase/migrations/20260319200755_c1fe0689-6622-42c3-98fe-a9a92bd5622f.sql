...
BEGIN
  ...
  ELSE
    -- For cash payments (both ILS and foreign), route to the box's GL account
    -- Foreign currencies will be tracked via foreign_amount column
  ...
  IF v_is_foreign THEN
    -- For foreign currency: create sale entry (ILS amount) debiting the box
    INSERT INTO public.transactions (
      ...
      'مبيعات نقطة البيع - ' || COALESCE(v_order.order_number, '') || ' (' || v_currency || ')',
      v_debit_account,
      ...
      v_foreign_amount, v_rate
    ) RETURNING id INTO v_tx_id;

    -- Record change given back in ILS (reduces the box ILS balance)
    v_change_ils := COALESCE((p_payments->0->>'change')::NUMERIC, 0);
    IF v_change_ils > 0 THEN
      INSERT INTO public.transactions (
        user_id, transaction_date, description,
        debit_account_code, credit_account_code,
        ...
        p_user_id, CURRENT_DATE,
        'باقي نقدي للزبون (عملة أجنبية) - ' || COALESCE(v_order.order_number, ''),
        '5900',
        v_box_gl_code,
        v_change_ils, 'شيكل', 'pos_change',
  ...
  ELSE
    INSERT INTO public.transactions (
      ...
      COALESCE(v_terminal.revenue_account_code, '4100'),
      v_order.total,
      CASE v_currency WHEN 'ILS' THEN 'شيكل' ELSE v_currency END,
  ...
  IF v_total_cogs > 0 THEN
    INSERT INTO public.transactions (
      ...
      COALESCE(v_terminal.inventory_account_code, '1400'),
