...
DECLARE
  ...
  v_max_session_minutes integer := 960;
...
BEGIN
  ...
  IF v_manual THEN
    ...
    -- A check-out outside the day window belongs to another day => missing check-out.
    IF v_last IS NOT NULL AND v_last >= v_end THEN
      v_last := NULL;
    END IF;
    IF v_first IS NOT NULL AND (v_first < v_start OR v_first >= v_end) THEN
      v_first := NULL;
    END IF;
