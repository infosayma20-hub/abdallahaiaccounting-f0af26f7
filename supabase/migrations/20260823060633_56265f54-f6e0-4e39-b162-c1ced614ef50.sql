...
DECLARE
  ...
  v_legacy boolean := p_date < DATE '2026-08-21'; -- تجميد السياسة القديمة للأيام المعتمدة سابقاً
  ...
  v_day_id uuid;
  -- سياسة المغادرات المدفوعة (company_settings)
...
BEGIN
  ...
  -- سياسة "المغادرات ضمن السقف مدفوعة" من إعدادات الشركة المالكة
  ...
  ELSE
    ...
    LOOP
      ...
      IF r.event_type = 'check_in' THEN
        ...
        IF v_session_start IS NULL THEN
          -- فجوة مغادرة بين جلستين مكتملتين (خروج سابق ← دخول جديد)
    ...
    -- المغادرات ضمن السقف مدفوعة: تُضاف لصافي الدوام فقط إن كانت السياسة مفعّلة
    -- والتاريخ ضمن السريان. الأيام المجمّدة (legacy) والأيام اليدوية لا تتأثر.
    IF v_paid_enabled AND NOT v_legacy AND p_date >= v_policy_from THEN
