-- Fix complete_pos_order: card payments should use bank account (1120), not cash box
...
BEGIN
  ...
  -- Get card/bank GL account from company settings
  ...
  ELSIF v_payment_method = 'card' THEN
    -- Card/Visa payments go to bank account, NOT cash box
...
-- Fix the two wrong transactions: change debit from 1110 (cash) to 1120 (bank) for card payments
UPDATE public.transactions
SET debit_account_code = '1120',
    account_id_debit = (SELECT id FROM public.accounts WHERE account_code = '1120' AND user_id = transactions.user_id LIMIT 1)
WHERE id IN ('259ba137-425a-43c8-8e07-e149e4be301b', 'f195358e-510c-4eec-88b4-47f32c7d15ac')
