-- ═══════════════════════════════════════════════════════════════════
-- POS Meal Subsidy: Company-paid portion of employee meals
-- Family discount = 10% (company pays 10, employee owes 90)
-- Individual discount = 50% (company pays 50, employee owes 50)
-- Gross sales stay intact; subsidy posts to 5316 against employee AR.
-- ═══════════════════════════════════════════════════════════════════

-- 1) Add column to track meal subsidy (company's share) on each order.
ALTER TABLE public.pos_orders
  ADD COLUMN IF NOT EXISTS meal_subsidy_amount NUMERIC NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS meal_discount_type TEXT;

COMMENT ON COLUMN public.pos_orders.meal_subsidy_amount IS
  'حصة الشركة من وجبة الموظف (تُرحّل إلى حساب 5316 وتُخصم من ذمة الموظف)';
COMMENT ON COLUMN public.pos_orders.meal_discount_type IS
  'نوع الخصم لطلبات حساب الموظف: family (10%) أو individual (50%)';

-- 2) Extend complete_pos_order with optional meal subsidy parameter.
--    Default 0 keeps all existing call sites backward-compatible.
CREATE OR REPLACE FUNCTION public.complete_pos_order(
  p_order_id UUID,
  p_user_id UUID,
  p_payments JSONB,
  p_meal_subsidy NUMERIC DEFAULT 0
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Meal subsidy guard: only valid on single-tender employee_account orders.
  ...
  -- ═══ NEW: Meal subsidy — company absorbs portion of employee meal ═══
  -- Dr 5316 (employee meals expense)  Cr [employee_account_code]
  -- Reduces employee AR while keeping gross sales (4100) intact.
