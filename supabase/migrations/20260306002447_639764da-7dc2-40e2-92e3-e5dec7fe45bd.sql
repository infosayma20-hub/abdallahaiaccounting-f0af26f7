...
BEGIN
  ...
  -- Extract currency info from first payment
  ...
  -- Determine debit account based on currency
  CASE v_currency
    WHEN 'USD' THEN v_debit_account := '1111'; -- صندوق الدولار
    WHEN 'JOD' THEN v_debit_account := '1112'; -- صندوق الدينار
    WHEN 'EUR' THEN v_debit_account := '1113'; -- صندوق اليورو
    WHEN 'EGP' THEN v_debit_account := '1114'; -- صندوق الجنيه
    ELSE v_debit_account := COALESCE(v_terminal.cash_account_code, '1110'); -- صندوق الشيكل
  END CASE;
  ...
  ELSE
    -- Foreign currency entry: debit foreign cash drawer with ILS equivalent
