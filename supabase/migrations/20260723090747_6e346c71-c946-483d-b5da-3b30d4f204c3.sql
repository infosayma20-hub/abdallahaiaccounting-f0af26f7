...
-- Fix `inconsistent_cash_payment_payload` false-positive for foreign-currency
-- payments with ILS (or foreign) change. Two bugs in the previous guard:
--   1) It compared `change` in raw units to a shekel amount, but the client
--      stores `change` in the unit of `change_currency` (ILS when change is
--      ILS, foreign units when change is USD/JOD).
--   2) Tolerance was 0.01 shekel — too tight for cashier rounding of change
--      on foreign-currency sales (e.g. 20 JOD @ 4.301 ≈ 86.02 ILS, invoice
--      85 ILS, cashier hands back exactly 1 ILS instead of 1.02).
--
-- Fix: convert `change` to ILS using `exchange_rate` when `change_currency`
-- is foreign, and allow up to 1 ILS rounding drift on cash tenders. All
-- other logic in complete_pos_order is unchanged.
...
DECLARE
  ...
  -- Payload integrity guard vars
  v_g_method TEXT; v_g_amount NUMERIC; v_g_tendered NUMERIC; v_g_change NUMERIC;
  v_g_change_cur TEXT; v_g_rate NUMERIC; v_g_change_ils NUMERIC; v_g_tolerance NUMERIC;
BEGIN
  ...
  -- 🛡️ PAYLOAD INTEGRITY GUARD (final source-of-truth defense).
  -- Cash-tender invariant: tendered_ILS - amount_ILS - change_ILS ≈ 0.
  -- `tendered` and `amount` are ALWAYS in shekels (POSPage builds them
  -- as `tendered * rate` before sending). `change` is in the unit of
  -- `change_currency`, so convert to shekels using `exchange_rate` when
  -- the change was returned in a foreign currency.
  --
  -- Tolerance: 1.00 ILS for cash tenders — cashiers routinely round
  -- foreign-currency change to a whole shekel/coin (e.g. 20 JOD paid
  -- for an 85 ILS invoice yields 1.02 ILS change, handed back as 1.00).
  -- 0.01 ILS was too tight and produced false-positive rejections.
  FOR v_payment IN SELECT * FROM jsonb_array_elements(p_payments) LOOP
    v_g_method     := COALESCE(v_payment.value->>'method', 'cash');
    v_g_amount     := COALESCE((v_payment.value->>'amount')::NUMERIC, 0);
    v_g_tendered   := COALESCE((v_payment.value->>'tendered')::NUMERIC, 0);
    v_g_change     := COALESCE((v_payment.value->>'change')::NUMERIC, 0);
    v_g_change_cur := COALESCE(v_payment.value->>'change_currency', 'ILS');
    v_g_rate       := COALESCE((v_payment.value->>'exchange_rate')::NUMERIC, 1);
    v_g_change_ils := CASE
      WHEN v_g_change_cur = 'ILS' OR v_g_rate <= 0 THEN v_g_change
      ELSE v_g_change * v_g_rate
    END;

    IF v_g_amount <= 0 THEN
      RETURN jsonb_build_object('success', false,
        'error', 'inconsistent_cash_payment_payload: amount must be positive');
    END IF;

    IF v_g_method = 'cash' THEN
      IF v_g_tendered <= 0 THEN
        RETURN jsonb_build_object('success', false,
          'error', 'inconsistent_cash_payment_payload: tendered must be positive for cash');
      END IF;
      -- Allow up to 1 ILS drift to accommodate cashier rounding of change
      -- on foreign-currency sales.
      v_g_tolerance := 1.0;
      IF ABS(v_g_tendered - v_g_amount - v_g_change_ils) > v_g_tolerance THEN
        RETURN jsonb_build_object('success', false,
          'error', format(
            'inconsistent_cash_payment_payload: tendered(%s) - amount(%s) - change_ils(%s) != 0',
            ROUND(v_g_tendered, 2), ROUND(v_g_amount, 2), ROUND(v_g_change_ils, 2)));
      END IF;
    END IF;
  END LOOP;
