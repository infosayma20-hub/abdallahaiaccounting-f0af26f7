CREATE OR REPLACE FUNCTION public.create_kiosk_call_center_order(
  ...
  p_payment_method text DEFAULT 'cashier',
...
BEGIN
  ...
  -- ── Daily order number, same design as the cashier's daily display number.
  -- Business day starts at 06:00 Asia/Hebron (same cutoff as pos_order_before_insert).
  ...
  SELECT GREATEST(
    -- highest cashier daily number today on this branch
    COALESCE((
    ...
    -- highest kiosk number already issued today on this branch
    COALESCE((
      SELECT MAX(NULLIF(regexp_replace(COALESCE(cco.delivery_info->>'order_number',''), '\D', '', 'g'), '')::int)
