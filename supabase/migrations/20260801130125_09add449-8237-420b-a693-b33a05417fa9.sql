...
BEGIN
  ...
  -- If one side is already the contact's own subledger, this is a transfer
  -- (e.g. merging one supplier balance into another). Do NOT rewrite the
  -- opposite side, otherwise both sides collapse onto the same account.
  IF NEW.debit_account_code = linked OR NEW.credit_account_code = linked THEN
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  -- Transfer between two subledgers: one side is already the contact's own
  -- account, so the other side must stay untouched (balance transfer / merge).
  IF NEW.debit_account_code = v_contact.linked_account_code THEN
    NEW.account_id_debit := v_linked_account.id;
    RETURN NEW;
  END IF;

  IF NEW.credit_account_code = v_contact.linked_account_code THEN
