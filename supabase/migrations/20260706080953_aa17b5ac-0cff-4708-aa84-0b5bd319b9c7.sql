...
-- Backfill existing manually-adjusted rows to point to the employee's real auth_user_id
UPDATE public.attendance_days ad
SET auth_user_id = e.auth_user_id, updated_at = now()
FROM public.employees e
WHERE ad.employee_id = e.id
  AND ad.is_manually_adjusted = true
  AND e.auth_user_id IS NOT NULL
  AND ad.auth_user_id IS DISTINCT FROM e.auth_user_id;

UPDATE public.attendance_events ae
SET auth_user_id = e.auth_user_id
FROM public.employees e
WHERE ae.employee_id = e.id
  AND ae.status = 'manual'
  AND e.auth_user_id IS NOT NULL
  AND ae.auth_user_id IS DISTINCT FROM e.auth_user_id;
