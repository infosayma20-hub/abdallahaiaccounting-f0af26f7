-- =====================================================================
-- POS Hardening: Visa app routing on payment edit + Cash-transfer linkage
-- =====================================================================

-- 1) Trigger to auto-link cash_transfers.pos_session_id when a transfer
--    originates from a cash box that has an OPEN pos_session — so shift
--    close can subtract that outflow. No behavior change when no open
--    session matches the box.
CREATE OR REPLACE FUNCTION public.tg_cash_transfers_autolink_pos_session()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  v_session_id uuid;
BEGIN
  IF NEW.pos_session_id IS NOT NULL THEN RETURN NEW; END IF;
  IF NEW.from_box_id IS NULL THEN RETURN NEW; END IF;

  SELECT id INTO v_session_id
    FROM public.pos_sessions
   WHERE user_id = NEW.user_id
     AND cash_box_id = NEW.from_box_id
     AND state = 'open'
   ORDER BY opened_at DESC NULLS LAST
   LIMIT 1;

  IF v_session_id IS NOT NULL THEN
    NEW.pos_session_id := v_session_id;
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_cash_transfers_autolink_pos_session ON public.cash_transfers;
CREATE TRIGGER trg_cash_transfers_autolink_pos_session
BEFORE INSERT ON public.cash_transfers
FOR EACH ROW EXECUTE FUNCTION public.tg_cash_transfers_autolink_pos_session();

-- 2) change_pos_payment_method — accept visa_gl_account_code for card routing
--    so switching a payment to Wheels/Yummy/FoodOnTime visa posts to the
--    correct AR account instead of the generic bank (1120).
--    Signature grows by ONE optional param at the end → safe for existing callers.
  ...
  BEGIN
    ...
    IF p_new_method <> 'mixed' THEN
      ...
      ELSIF p_new_method = 'card' THEN
        -- 🔥 Honor visa_gl_account_code for delivery-app visa routing (Wheels/Yummy/FoodOnTime)
      ...
      UPDATE public.transactions
         SET debit_account_code = v_new_debit_account, payment_method = p_new_method, updated_at = now(),
             notes = COALESCE(notes,'') || format(
               E'\n[gl-sync %s] method:%s->%s cur:%s->%s visa_gl:%s by:%s reason:%s',
...
-- 3) Backfill: link existing cash_transfers made from a currently-open
--    session's cash box to that session (so today's expected cash reflects
--    those outflows once the cashier hits Close).
UPDATE public.cash_transfers ct
   SET pos_session_id = ps.id
  FROM public.pos_sessions ps
 WHERE ct.pos_session_id IS NULL
   AND ct.from_box_id IS NOT NULL
   AND ps.state = 'open'
   AND ps.cash_box_id = ct.from_box_id
   AND ps.user_id = ct.user_id
   AND ct.created_at >= ps.opened_at;