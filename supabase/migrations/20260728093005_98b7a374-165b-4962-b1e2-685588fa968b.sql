...
BEGIN
  ...
  -- Prefer KDS-assigned display number when present, otherwise use MAX+1
  -- (MAX is stable across rollbacks — COUNT+1 was fragile with gaps).
  ...
  -- Build candidate order_number and retry-bump on collision (protects against
  -- KDS/branch/shift stamping races that could produce the same order_number
  -- for two rows in the same tenant on the same business day).
  LOOP
    ...
    IF v_attempts > 50 THEN
      -- Extremely unlikely; fall through and let the unique index surface the error
      EXIT;
    END IF;
  END LOOP;

  NEW.order_number := v_candidate;
  -- Keep daily_display_number in sync with the final chosen number so KDS
