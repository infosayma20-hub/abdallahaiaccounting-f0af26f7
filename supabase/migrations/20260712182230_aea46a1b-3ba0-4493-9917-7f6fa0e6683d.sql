...
BEGIN
  ...
  -- Resolve branch of the session's cash box (for per-currency box validation).
  ...
  IF p_new_method = 'mixed' THEN
    FOR v_split_row IN SELECT * FROM jsonb_array_elements(p_split_payments) LOOP
      ...
      -- Per-line FX consistency: foreign_amount * exchange_rate ≈ amount(ILS)
      ...
      ELSE
        ...
        IF abs((v_line_foreign * v_line_rate) - v_line_amount) > 0.01 THEN
          RAISE EXCEPTION 'SPLIT_FX_MISMATCH' USING ERRCODE = 'P0001',
            HINT = format('عدم تطابق: %s %s × %s ≠ %s ₪',
                          v_line_foreign, v_line_currency, v_line_rate, v_line_amount); END IF;
      ...
      -- For cash lines in a foreign currency, require an actual per-currency cash box for the branch.
  ...
  IF p_new_currency IS NOT NULL THEN
    IF p_new_method = 'mixed' THEN
      -- p_new_currency ignored for mixed — per-line currency is used instead.
      NULL;
    ELSE
  ...
  IF p_new_method = 'mixed' THEN
    ...
    FOR v_split_row IN SELECT * FROM jsonb_array_elements(p_split_payments) LOOP
      ...
      VALUES (v_company_id, p_order_id, v_split_row->>'method',
        ...
        upper(COALESCE(v_split_row->>'change_currency', v_line_currency)),
