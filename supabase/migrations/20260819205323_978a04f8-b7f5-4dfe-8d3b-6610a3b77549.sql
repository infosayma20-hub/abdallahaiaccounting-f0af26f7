  ...
  WITH lines AS (
    SELECT t.id, t.transaction_date, t.description, t.reference, t.transaction_type, t.created_at,
           CASE
             ...
             WHEN t.debit_account_code LIKE '113%' OR t.debit_account_code LIKE '21%' THEN t.amount
             ELSE 0
           END AS debit,
           CASE
             WHEN t.credit_account_code = v_code THEN t.amount
             WHEN t.debit_account_code = v_code THEN 0
             WHEN t.credit_account_code LIKE '113%' OR t.credit_account_code LIKE '21%' THEN t.amount
             ELSE 0
           END AS credit
