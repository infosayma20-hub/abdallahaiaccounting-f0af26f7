...
-- Link kiosk_settings to a specific bank account (Visa) so kiosk-paid card orders route to that bank's GL
ALTER TABLE public.kiosk_settings
  ADD COLUMN IF NOT EXISTS visa_bank_account_id uuid NULL REFERENCES public.bank_accounts(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_kiosk_settings_visa_bank ON public.kiosk_settings(visa_bank_account_id) WHERE visa_bank_account_id IS NOT NULL;

-- Recreate RPC: populate visa_gl_account_code from linked bank account when payment is visa
...
BEGIN
  ...
  -- Resolve visa GL from linked bank account (Bank of Palestine Visa) when payment is visa
