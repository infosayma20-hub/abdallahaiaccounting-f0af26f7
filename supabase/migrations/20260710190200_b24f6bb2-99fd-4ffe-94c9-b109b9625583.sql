...
CREATE OR REPLACE FUNCTION public.get_owner_sales_fast(
  p_user_id uuid,
  p_from date,
  p_to date,
  p_with_details boolean DEFAULT true
)
RETURNS jsonb
LANGUAGE sql
STABLE
    ...
    GREATEST(
      ...
      COALESCE(o.total, 0)::numeric
        - CASE WHEN COALESCE(o.total_includes_delivery_fee, false)
          THEN COALESCE(o.delivery_fee, 0)::numeric
          ELSE 0::numeric
        END
    ) AS net_total
...
order_enriched AS MATERIALIZED (
  SELECT
    ...
    CASE
      WHEN COALESCE(cb.branch_id, pt.branch_id) IS NULL THEN 'بدون فرع'
      ELSE COALESCE(br.name, 'فرع غير مسمى')
    END AS branch_name,
...
payments_adjusted AS MATERIALIZED (
  SELECT
    o.id AS order_id,
    CASE
      WHEN o.total_includes_delivery_fee THEN GREATEST(0, COALESCE(p.cash, 0) - LEAST(COALESCE(p.cash, 0), o.delivery_fee))
      ELSE COALESCE(p.cash, 0)
    END::numeric AS cash,
    CASE
      WHEN o.total_includes_delivery_fee THEN GREATEST(0, COALESCE(p.card, 0) - GREATEST(0, o.delivery_fee - COALESCE(p.cash, 0)))
      ELSE COALESCE(p.card, 0)
    END::numeric AS card,
...
branch_cancel AS (
  ...
  FROM cancelled_enriched e
  GROUP BY e.branch_key
),
branch_keys AS (
  SELECT branch_key FROM branch_paid
  UNION
  SELECT branch_key FROM branch_cancel
),
branch_rows AS (
  SELECT
    k.branch_key AS id,
  ...
  UNION ALL
  SELECT
    '__invoices__', 'فواتير المبيعات', 'المحاسبة', it.inv_total, it.inv_count,
    it.inv_total, it.inv_total, 0, 0, 0, 0, 0, 0
...
cashier_paid AS (
  SELECT
    e.branch_key,
    MIN(e.branch_name) AS branch_name,
    e.cashier_name,
    COALESCE(SUM(e.net_total), 0)::numeric AS total,
    COUNT(*)::integer AS order_count,
    COALESCE(SUM(p.cash), 0)::numeric AS cash,
...
cashier_keys AS (
  ...
  UNION
  SELECT branch_key, cashier_name FROM cashier_cancel
),
cashier_rows AS (
  SELECT
...
GRANT EXECUTE ON FUNCTION public.get_owner_sales_fast(uuid, date, date, boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_owner_sales_fast(uuid, date, date, boolean) TO service_role;