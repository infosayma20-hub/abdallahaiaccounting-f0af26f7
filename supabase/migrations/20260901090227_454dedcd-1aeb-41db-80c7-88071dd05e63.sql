...
BEGIN
  ...
  -- TENANT-SCOPED lookup by STABLE employee_id link on the sub-ledger account.
  -- Never match by account_name text: name variations silently dropped movements.
  SELECT a.employee_id, (a.account_code = NEW.debit_account_code)
    INTO v_emp_id, v_is_debit
  FROM public.accounts a
  WHERE a.employee_id IS NOT NULL
    AND a.user_id = NEW.user_id
    AND a.account_code IN (NEW.debit_account_code, NEW.credit_account_code)
  ORDER BY (a.account_code = NEW.debit_account_code) DESC
  LIMIT 1;

  -- Legacy fallback: accounts not yet linked to an employee_id
