...
BEGIN
  ...
  -- balances from transactions ledger only
  WITH t AS (
  ...
  SELECT COALESCE(jsonb_agg(row_to_json(x)), '[]'::jsonb) INTO v_recent_vouchers FROM (
    SELECT id, voucher_number, voucher_date, voucher_type, amount, contact_id, contact_name
    FROM (
      SELECT id, voucher_number, voucher_date, 'receipt'::text AS voucher_type, amount, contact_id, contact_name, created_at
      FROM receipt_vouchers WHERE user_id=v_user
      UNION ALL
      SELECT id, voucher_number, voucher_date, 'payment'::text, amount, contact_id, contact_name, created_at
      FROM payment_vouchers WHERE user_id=v_user
    ) u ORDER BY created_at DESC LIMIT 10
  ) x;
...
GRANT EXECUTE ON FUNCTION public.get_accounting_center_snapshot() TO authenticated;