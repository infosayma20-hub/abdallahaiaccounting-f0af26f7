CREATE OR REPLACE FUNCTION public.get_accounting_center_kpi_breakdown(_prefix text, _natural text DEFAULT 'debit')
RETURNS jsonb
LANGUAGE plpgsql
STABLE
...
BEGIN
  ...
  FROM (
    SELECT a.code, COALESCE(acc.name, a.code) AS name, a.total_debit, a.total_credit, a.balance, a.entries
    FROM agg a
    LEFT JOIN accounts acc ON acc.code = a.code AND acc.user_id = v_user
  ) x;
...
GRANT EXECUTE ON FUNCTION public.get_accounting_center_kpi_breakdown(text, text) TO authenticated, service_role;