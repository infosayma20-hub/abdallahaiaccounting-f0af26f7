...
BEGIN
  ...
  -- Look up cash box GL account if assigned to session
  IF v_order.cash_box_id IS NOT NULL THEN
    SELECT gl_account_code INTO v_cash_box_gl
    FROM public.cash_boxes
    WHERE id = v_order.cash_box_id;
  END IF;
  ...
  ELSE
    CASE v_currency
      ...
      ELSE v_debit_account := COALESCE(v_cash_box_gl, v_terminal.cash_account_code, '1110');
    END CASE;
  END IF;
