-- ============================================================================
-- Safe incremental fix for create_rep_sale_atomic (discount overload)
-- 1) Cash sales: post to rep's cash box (p_cash_account_code), not 1115
-- 2) sales_rep_id stamped on sale tx (both cash & credit), COGS, discount
-- Everything else (stock_movements, invoice_items, products.quantity,
-- buy_price, validation, discount math, invoice_number) is UNCHANGED.
-- ============================================================================

DROP FUNCTION IF EXISTS public.create_rep_sale_atomic(
  uuid, uuid, uuid, uuid, uuid, text, text, jsonb, text, text, text, numeric
);
...
BEGIN
  ...
  -- Pre-pass: totals + cost + stock validation (UNCHANGED)
  ...
  -- ============================================================
  -- Sale GL entry (CHANGED branch)
  -- Cash: Dr p_cash_account_code (rep's box) / Cr 4110, direct insert
  -- Credit: keep create_invoice_with_entry, then stamp sales_rep_id
  -- ============================================================
  ...
  ELSE
    ...
    -- Stamp sales_rep_id (meta field, no financial impact)
    UPDATE public.transactions
       SET sales_rep_id = COALESCE(sales_rep_id, p_sales_rep_id)
     WHERE id = v_tx_id;
  END IF;

  -- Invoice header (UNCHANGED)
  ...
  -- Discount tx (UNCHANGED math; +sales_rep_id)
  ...
  -- COGS tx (UNCHANGED math; +sales_rep_id)
