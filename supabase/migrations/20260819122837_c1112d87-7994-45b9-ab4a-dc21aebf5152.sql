DROP FUNCTION IF EXISTS public.get_form_audience(text, text, uuid);
DROP FUNCTION IF EXISTS public.get_form_catalog(text[]);

-- ---------- Catalog: all forms + audience counts ----------
CREATE OR REPLACE FUNCTION public.get_form_catalog(
  p_builtin_keys text[] DEFAULT NULL,
  p_manager_only_keys text[] DEFAULT NULL
)
RETURNS TABLE(
  kind text,
  form_key text,
...
BEGIN
  ...
  WITH tenant_emp AS (
    SELECT e.id, e.job_title, e.auth_user_id, jt.name AS jt_name,
           EXISTS (SELECT 1 FROM public.branch_manager_assignments bma WHERE bma.user_id = e.auth_user_id) AS is_bm
...
BEGIN
  ...
  calc AS (
    SELECT
      ...
      -- Builtin defaults that already exist in the app today:
      --   * general builtin forms  -> every employee can fill
      --   * manager-only builtin   -> only branch managers can fill
      CASE
  ...
  SELECT
    ...
    (c.manual_fill OR c.manual_view OR c.by_title OR c.default_fill) AS can_view,
    CASE
    ...
    END AS fill_source,
    CASE
      WHEN c.default_fill THEN 'default'
...
REVOKE EXECUTE ON FUNCTION public.get_form_catalog(text[], text[]) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.get_form_audience(text, text, uuid, boolean) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_form_catalog(text[], text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_form_audience(text, text, uuid, boolean) TO authenticated;