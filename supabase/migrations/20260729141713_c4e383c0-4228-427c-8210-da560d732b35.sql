CREATE INDEX IF NOT EXISTS idx_pos_orders_reports_owner_day_branch_created
  ON public.pos_orders (user_id, business_date, branch_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_pos_sessions_reports_owner_day_branch
  ON public.pos_sessions (user_id, business_date, branch_id)
  WHERE is_deleted = false;

CREATE INDEX IF NOT EXISTS idx_pos_order_lines_reports_order_cost
  ON public.pos_order_lines (order_id)
  INCLUDE (qty, cost_price, total, discount_amount);

CREATE INDEX IF NOT EXISTS idx_pos_payments_reports_order_method_amount
  ON public.pos_payments (order_id, payment_method)
  INCLUDE (amount, created_at);

CREATE OR REPLACE FUNCTION public.get_pos_reports_summary(
  p_from date,
  p_to date,
...
orders_scope AS (
  ...
  LEFT JOIN public.transactions t1 ON t1.id = o.transaction_id
  LEFT JOIN public.transactions t2 ON t2.id = o.linked_transaction_id
  CROSS JOIN owner
  WHERE auth.uid() IS NOT NULL
    ...
    AND o.state <> 'draft'
    AND COALESCE(t1.is_deleted, false) = false
...
REVOKE ALL ON FUNCTION public.get_pos_reports_summary(date, date, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_pos_reports_summary(date, date, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_pos_reports_summary(date, date, uuid) TO service_role;