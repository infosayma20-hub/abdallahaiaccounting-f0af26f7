...
-- FIX 1: Payroll trigger uses 5100 (COGS) instead of 5150 (Salaries)
...
BEGIN
  IF NEW.is_paid = true AND (OLD.is_paid = false OR OLD.is_paid IS NULL) THEN
    ...
    INSERT INTO public.transactions (
      ...
      '5150', -- Fixed: was 5100 (COGS), now 5150 (رواتب وأجور)
      '1110',
      NEW.net_salary,
      'شيكل',
      'salary',
...
-- FIX 2: Purchase function uses 2100 (doesn't exist) instead of 2110
...
BEGIN
  ...
  CASE p_payment_method
    ...
    ELSE v_credit_code := '2110'; v_tx_type := 'purchase_credit'; -- Fixed: was 2100
  END CASE;
...
-- FIX 3: Cheque collection trigger uses 2100 instead of 2110
...
BEGIN
  IF NEW.status = 'محصل' AND OLD.status != 'محصل' THEN
    IF NEW.cheque_type = 'صادر' THEN
      v_debit := '2110'; -- Fixed: was 2100 (doesn't exist), now 2110 (ذمم موردين)
      v_credit := '1120';
    ...
    INSERT INTO public.transactions (
      ...
      NEW.user_id, CURRENT_DATE, v_desc,
      v_debit, v_credit, NEW.amount,
...
-- FIX 4: POS complete_pos_order uses 1400 (doesn't exist) for inventory fallback
-- Fix the COGS inventory account from 1400 to 1140
...
BEGIN
  ...
  IF v_total_cogs > 0 THEN
    INSERT INTO public.transactions (
      ...
      COALESCE(v_terminal.inventory_account_code, '1140'), -- Fixed: was 1400
