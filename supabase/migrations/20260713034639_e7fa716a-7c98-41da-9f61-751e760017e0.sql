...
BEGIN
  ...
  -- Rejects any cash tender whose (tendered - amount - change) drifts more than
  -- 0.01 ILS. The client always sends `tendered` and `change` in ILS units
  -- (see POSPage.tsx handleCompleteOrder), so the invariant is straightforward.
  -- This blocks the class of bugs where a stale foreign-currency selection in
  -- the payment dialog leaks a wrong `currency` / `tendered` combination into
  -- pos_payments — which then inflates the shift-close breakdown by a currency
  ...
  FOR v_payment IN SELECT * FROM jsonb_array_elements(p_payments) LOOP
    ...
    IF v_g_method = 'cash' THEN
      ...
      END IF;
      IF ABS(v_g_tendered - v_g_amount - v_g_change) > 0.01 THEN
        RETURN jsonb_build_object('success', false,
          'error', format(
            'inconsistent_cash_payment_payload: tendered(%s) - amount(%s) - change(%s) != 0',
            v_g_tendered, v_g_amount, v_g_change));
      END IF;
    END IF;
  END LOOP;
