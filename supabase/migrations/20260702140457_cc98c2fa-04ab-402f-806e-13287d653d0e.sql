...
BEGIN
  ...
  IF p_new_method = 'mixed' THEN
    ...
    FOR v_split_row IN SELECT * FROM jsonb_array_elements(p_split_payments) LOOP
      ...
      VALUES (v_company_id, p_order_id, v_split_row->>'method',
        ...
        0, 'ILS', 1, v_old_methods, now(), p_pos_user_id, p_manager_user_id, p_edit_reason);
  ...
  IF p_new_currency IS NOT NULL AND p_new_method <> 'mixed' THEN
    IF upper(p_new_currency) = 'ILS' THEN
      ...
      UPDATE pos_orders
         SET payment_currency='ILS',
             payment_currency_amount=v_new_amount,
    ...
    ELSE
      ...
      UPDATE pos_payments SET currency=upper(p_new_currency), exchange_rate=p_new_exchange_rate,
       ...
       WHERE order_id = p_order_id;
      UPDATE pos_orders
         SET payment_currency=upper(p_new_currency),
             payment_currency_amount=v_new_amount,
             payment_currency_rate=p_new_exchange_rate
       WHERE id = p_order_id;
    END IF;
  END IF;
  ...
  IF p_new_method = 'mixed' THEN
    UPDATE pos_orders
       SET payment_currency='ILS',
           payment_currency_amount=COALESCE(v_order.total, 0),
           payment_currency_rate=1
     WHERE id = p_order_id;
  END IF;
  ...
  DECLARE
    ...
    v_gl_rows_updated int := 0;
    v_gl_rows_deleted int := 0;
    v_tender jsonb;
    v_tender_idx int := 0;
    v_n_new_tenders int;
    v_new_debit text;
    v_new_rev_acc text := '4100';
    v_new_vat_acc text;
    v_terminal record;
    v_box_gl_code text;
  ...
  BEGIN
    ...
    SELECT credit_account_code INTO v_new_vat_acc
     ...
     WHERE user_id = v_company_id
       ...
       AND transaction_type = 'pos_sale_vat'
    ...
    IF p_new_method <> 'mixed' THEN
      IF p_new_method = 'credit' THEN
        v_new_debit_account := '1130';
      ELSIF p_new_method = 'card' THEN
        v_new_debit_account := v_card_bank_gl;
      ELSIF p_new_method = 'employee_account' THEN
        SELECT COALESCE(linked_account_code, '2180')
          INTO v_new_debit_account
      ...
      UPDATE public.transactions
         SET debit_account_code = v_new_debit_account,
             payment_method = p_new_method,
             updated_at = now(),
             notes = COALESCE(notes,'') || format(
               ...
               COALESCE(p_pos_user_id::text,'-'),
               COALESCE(p_edit_reason,'-'))
       WHERE user_id = v_company_id
         AND reference = v_order.order_number
    ...
    ELSE
      ...
      FOR v_tender IN SELECT * FROM jsonb_array_elements(p_split_payments) LOOP
        ...
        ELSE v_new_debit := public._pos_resolve_cash_gl(v_order.cash_box_id, 'ILS', v_box_gl_code);
        END IF;
        ...
        IF v_tender_net > 0 THEN
          ...
          VALUES (v_company_id, CURRENT_DATE,
            ...
            'شيكل', 'pos_sale', v_customer_id, v_order.order_number,
            v_tender->>'method',
        ...
        IF v_tender_vat > 0 AND v_new_vat_acc IS NOT NULL THEN
          ...
          VALUES (v_company_id, CURRENT_DATE,
            ...
            'شيكل', 'pos_sale_vat', v_customer_id, v_order.order_number,
            v_tender->>'method',
  ...
  VALUES (v_company_id,
    ...
    jsonb_build_object(
      ...
      'header_currency_synced', true,
      'gl_sync_applied', true));
