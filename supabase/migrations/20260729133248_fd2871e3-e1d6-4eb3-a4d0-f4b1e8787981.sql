CREATE OR REPLACE FUNCTION public.pos_normalize_delivery_app(p_text text)
RETURNS text
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT CASE
    ...
    WHEN p_text ILIKE '%شني%' OR p_text ILIKE '%shini%' THEN 'Shini Go'
    WHEN p_text ILIKE '%يمي%' OR p_text ILIKE '%yummy%' THEN 'Yummy'
    WHEN p_text ILIKE '%اون تايم%' OR p_text ILIKE '%foodontime%' OR p_text ILIKE '%food on time%' THEN 'FoodOnTime'
    WHEN p_text ILIKE '%بال ايت%' OR p_text ILIKE '%بالايت%' OR p_text ILIKE '%pal eat%' THEN 'بال ايت'
    WHEN p_text ILIKE '%ويلز%' OR p_text ILIKE '%wheels%' THEN 'Wheels'
    ELSE NULL
  END
$$;
...
BEGIN
  ...
  ord AS (
    SELECT
      ...
      COALESCE(
        public.pos_normalize_delivery_app(cc.source_app),
...
GRANT EXECUTE ON FUNCTION public.get_delivery_apps_report(date, date, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.pos_normalize_delivery_app(text) TO authenticated;