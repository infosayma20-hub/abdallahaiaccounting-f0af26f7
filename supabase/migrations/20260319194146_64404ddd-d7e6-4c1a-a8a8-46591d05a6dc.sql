...
ALTER TABLE public.pos_payments ADD COLUMN IF NOT EXISTS exchange_rate NUMERIC DEFAULT 1;

-- Update the complete_pos_order function to store exchange_rate
...
BEGIN
  ...
  IF v_is_foreign THEN
    ...
    INSERT INTO public.transactions (
      ...
      p_user_id, CURRENT_DATE,
      'صرف عملة - ' || v_currency || ' ← شيكل (POS)',
      v_debit_account,
      COALESCE(v_terminal.cash_account_code, '1110'),
      v_order.total + v_change_ils, 'شيكل', 'pos_exchange',
