...
BEGIN
  ...
  IF p_check_in IS NULL OR p_check_out IS NULL THEN
    RAISE EXCEPTION 'check_in and check_out are required';
  END IF;
  ...
  SELECT e.id, e.user_id, e.branch_id, e.full_name, e.is_active
    INTO v_emp
    ...
    BEGIN
      SELECT COUNT(*) INTO v_existing_events
       ...
       WHERE ae.employee_id = p_employee_id
         AND (ae.event_time AT TIME ZONE 'UTC')::date = v_day;

      IF v_existing_events > 0 OR EXISTS (
      ...
      ELSE
        v_check_in_ts := (v_day::timestamp + p_check_in)::timestamptz;
        v_check_out_ts := (v_day::timestamp + p_check_out)::timestamptz;
        IF v_check_out_ts <= v_check_in_ts THEN
          v_check_out_ts := v_check_out_ts + interval '1 day';
        END IF;
        v_total_hours := ROUND(EXTRACT(EPOCH FROM (v_check_out_ts - v_check_in_ts)) / 3600.0, 2);
        v_status := 'present';

        INSERT INTO public.attendance_events(
          employee_id, auth_user_id, branch_id, event_type, event_time,
          status, notes, server_recorded
        ) VALUES (
          p_employee_id, v_emp.user_id, v_emp.branch_id, 'check_in', v_check_in_ts,
          'manual', p_reason, true
        ) RETURNING id INTO v_event_in_id;

        INSERT INTO public.attendance_events(
          employee_id, auth_user_id, branch_id, event_type, event_time,
          status, notes, server_recorded
        ) VALUES (
          p_employee_id, v_emp.user_id, v_emp.branch_id, 'check_out', v_check_out_ts,
          'manual', p_reason, true
        ) RETURNING id INTO v_event_out_id;
        ...
        INSERT INTO public.attendance_days(
          ...
          p_employee_id, v_emp.user_id, v_emp.branch_id, v_day,
          v_check_in_ts, v_check_out_ts, v_total_hours, v_status,
          true, p_reason
        )
        ...
        INSERT INTO public.attendance_audit_logs(
          ...
          'attendance_days', v_day_id, 'insert',
          jsonb_build_object(
...
REVOKE ALL ON FUNCTION public.hr_backfill_attendance(uuid, date, date, time, time, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.hr_backfill_attendance(uuid, date, date, time, time, text) TO authenticated;
