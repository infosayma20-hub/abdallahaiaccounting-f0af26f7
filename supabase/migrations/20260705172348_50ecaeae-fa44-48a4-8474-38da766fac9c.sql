...
CREATE OR REPLACE FUNCTION public.hr_backfill_attendance(
  ...
  p_check_in time without time zone, p_check_out time without time zone,
  p_reason text, p_overwrite boolean DEFAULT false)
...
BEGIN
  ...
  -- Resolve branch: employee.branch_id → first allowed branch → any branch owned by same user
  ...
  WHILE v_day <= p_to LOOP
    BEGIN
      ...
      ELSE
        ...
        -- server_recorded=false so enforce_server_event_time does NOT overwrite event_time
        INSERT INTO public.attendance_events(
          employee_id, auth_user_id, branch_id, event_type, event_time, status, notes, server_recorded
        ) VALUES (
          p_employee_id, v_emp.user_id, v_branch_id, 'check_in', v_check_in_ts, 'manual', p_reason, false
        ) RETURNING id INTO v_event_in_id;

        INSERT INTO public.attendance_events(
          employee_id, auth_user_id, branch_id, event_type, event_time, status, notes, server_recorded
        ) VALUES (
          p_employee_id, v_emp.user_id, v_branch_id, 'check_out', v_check_out_ts, 'manual', p_reason, false
        ...
        INSERT INTO public.attendance_days(
          ...
          p_employee_id, v_emp.user_id, v_branch_id, v_day,
