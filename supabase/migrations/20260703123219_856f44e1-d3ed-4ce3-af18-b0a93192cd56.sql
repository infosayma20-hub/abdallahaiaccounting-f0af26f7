...
DROP FUNCTION IF EXISTS public.repair_contact_accounts_for_tenant(uuid, boolean);

CREATE FUNCTION public.repair_contact_accounts_for_tenant(
  p_user_id uuid,
  p_dry_run boolean DEFAULT true
)
RETURNS TABLE(
  r_action text, r_entity text, r_before text, r_after text,
  r_rows integer, r_amount numeric, r_notes text
)
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
DECLARE
  v_batch text := 'contact_repair_' || to_char(now(),'YYYYMMDDHH24MISS') || '_' || SUBSTRING(p_user_id::text,1,8);
  v_contact record; v_target_parent text; v_target_code text; v_ar_code text;
  v_moved int; v_amt numeric; v_tx record; v_new_code text;
BEGIN
  FOR v_contact IN
  ...
  LOOP
    ...
    SELECT a.account_code INTO v_target_code FROM public.accounts a
    WHERE a.user_id = p_user_id AND a.parent_code = v_target_parent
      AND (a.contact_id = v_contact.id OR a.account_name = v_contact.contact_name)
      AND a.is_active
    ORDER BY (a.contact_id = v_contact.id) DESC NULLS LAST LIMIT 1;

    IF v_target_code IS NULL THEN
      IF NOT p_dry_run THEN
        v_target_code := public.resolve_postable_account(p_user_id, v_target_parent, v_contact.id, v_contact.contact_name, v_contact.contact_type);
      ELSE v_target_code := v_target_parent || '(new)';
      END IF;
    END IF;
    ...
    IF NOT p_dry_run THEN
      ...
      UPDATE public.accounts a SET is_active = false,
        notes = COALESCE(a.notes,'') || E'\n[REPAIRED ' || v_batch || '] merged into ' || v_target_code
      WHERE a.user_id = p_user_id AND a.account_code = v_ar_code AND COALESCE(a.is_system_protected,false)=false;
    ...
    RETURN QUERY SELECT 'fix_wrong_root'::text,
      v_contact.contact_name || ' (' || v_contact.contact_type || ')',
      v_ar_code, v_target_code, v_moved, v_amt,
  ...
  FOR v_tx IN
    ...
    FROM public.transactions t
    WHERE t.user_id = p_user_id AND NOT t.is_deleted AND t.contact_id IS NOT NULL
  ...
  LOOP
    IF v_tx.debit_account_code IN ('1130','2110','2180','1146') THEN
      IF NOT p_dry_run THEN
        BEGIN
          v_new_code := public.resolve_postable_account(p_user_id, v_tx.debit_account_code, v_tx.contact_id, NULL, NULL);
          IF v_new_code <> v_tx.debit_account_code THEN
            ...
            VALUES (v_batch, 'tx_reroute_debit', v_tx.id,
              ...
              'Parent-root debit rerouted to leaf');
          END IF;
        EXCEPTION WHEN OTHERS THEN v_new_code := NULL; END;
      ELSE v_new_code := NULL; END IF;
      RETURN QUERY SELECT 'reroute_debit'::text, v_tx.reference,
        ...
        1, v_tx.amount, 'Debit rerouted';
    END IF;

    IF v_tx.credit_account_code IN ('1130','2110','2180','1146') THEN
      IF NOT p_dry_run THEN
        BEGIN
          v_new_code := public.resolve_postable_account(p_user_id, v_tx.credit_account_code, v_tx.contact_id, NULL, NULL);
          IF v_new_code <> v_tx.credit_account_code THEN
            ...
            VALUES (v_batch, 'tx_reroute_credit', v_tx.id,
              ...
              'Parent-root credit rerouted to leaf');
          END IF;
        EXCEPTION WHEN OTHERS THEN v_new_code := NULL; END;
      ELSE v_new_code := NULL; END IF;
      RETURN QUERY SELECT 'reroute_credit'::text, v_tx.reference,
        ...
        1, v_tx.amount, 'Credit rerouted';
    END IF;
  END LOOP;

  RETURN QUERY
  SELECT 'MANUAL_REVIEW_NEEDED'::text,
    ...
    NULL::text, 1, t.amount, 'No contact_id - manual review'
  FROM public.transactions t
  WHERE t.user_id = p_user_id AND NOT t.is_deleted AND t.contact_id IS NULL
    AND (t.debit_account_code IN ('1130','2110','2180','1146')
         OR t.credit_account_code IN ('1130','2110','2180','1146'));
...
END;
$function$;
GRANT EXECUTE ON FUNCTION public.repair_contact_accounts_for_tenant(uuid, boolean) TO authenticated, service_role;
