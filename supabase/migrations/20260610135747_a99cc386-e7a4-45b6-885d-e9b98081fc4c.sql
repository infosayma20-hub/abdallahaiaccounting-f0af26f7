...
BEGIN
  ...
  -- FIX: pos_orders.user_id is the tenant id (matches device.company_id),
  -- while pos_orders.company_id is the pos_companies id (different).
  ...
  IF _updated = 0 THEN
    SELECT id INTO _ticket_id FROM public.kitchen_tickets
     WHERE order_id = _order_id ORDER BY created_at ASC LIMIT 1;
