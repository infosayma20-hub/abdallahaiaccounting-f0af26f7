...
BEGIN
  ...
  -- Authoritative span rule:
  --  * Manually adjusted days (HR curated the in/out times and the session list):
  --    net = (last_check_out - first_check_in) - all recorded sessions. Stray or
  --    late punches inside the curated span must not carve out unpaid holes.
  --  * Days with no usable punch pairs fall back to the same span rule.
  IF (v_manual OR v_total_minutes = 0)
     AND v_first IS NOT NULL AND v_last IS NOT NULL AND v_last > v_first THEN
