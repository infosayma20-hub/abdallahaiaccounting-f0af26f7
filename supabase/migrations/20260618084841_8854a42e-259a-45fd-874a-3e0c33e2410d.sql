...
-- Fix notification inserts to match real column schemas
...
BEGIN
  ...
  -- admin_notifications schema: (user_id, event_type, metadata)
  BEGIN
    INSERT INTO public.admin_notifications (user_id, event_type, metadata)
    VALUES (
      v_inv.user_id, 'rep_edit_request',
      jsonb_build_object(
        'invoice_id', p_invoice_id,
        'invoice_number', v_inv.invoice_number,
        'request_id', v_request_id,
        'link', '/admin/rep-edit-requests',
        'title', 'طلب تعديل فاتورة مندوب',
        'message', 'طلب تعديل على الفاتورة ' || v_inv.invoice_number
      )
    );
...
BEGIN
  ...
  -- notification_log schema: (user_id, type, title, body, path)
  BEGIN
    INSERT INTO public.notification_log (user_id, type, title, body, path)
    VALUES (
      v_req.requested_by, 'rep_edit_rejected',
      'تم رفض طلب التعديل',
      COALESCE('السبب: ' || NULLIF(trim(COALESCE(p_note,'')),''), 'تم رفض طلب تعديلك'),
      '/rep/orders'
    );
  EXCEPTION WHEN OTHERS THEN NULL; END;
...
BEGIN
  ...
  BEGIN
    INSERT INTO public.notification_log (user_id, type, title, body, path)
    VALUES (
      v_req.requested_by, 'rep_edit_approved',
