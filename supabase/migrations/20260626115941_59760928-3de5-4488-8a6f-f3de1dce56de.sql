...
DECLARE
  ...
  -- Per-tender currency context (mixed-currency support)
...
BEGIN
  ...
  IF v_n_tenders > 1 THEN
    -- Only cash + card combinations allowed in split mode
    IF EXISTS (
    ...
    END IF;
    -- Foreign currency tenders MUST be cash (no foreign card swipes to avoid bank/settlement issues)
    IF EXISTS (
      SELECT 1 FROM jsonb_array_elements(p_payments) p
  ...
  FOR v_payment IN SELECT * FROM jsonb_array_elements(p_payments) LOOP
    ...
    -- Per-tender currency context (allows mixed-currency split payments)
