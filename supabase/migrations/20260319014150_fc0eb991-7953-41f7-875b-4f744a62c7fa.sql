...
BEGIN
  ...
  -- Determine debit account for the payment
  ...
  IF v_is_foreign THEN
    -- ======= MULTI-CURRENCY FLOW (3 entries) =======
    
    -- 1) Sales entry: Debit ILS cash box → Credit Revenue (always in ILS)
    ...
    -- 2) Currency exchange: Debit foreign cash box → Credit ILS cash box
    --    Amount = ILS equivalent of foreign received, foreign_amount = actual foreign
    INSERT INTO public.transactions (
      user_id, transaction_date, description,
      debit_account_code, credit_account_code,
      ...
      p_user_id, CURRENT_DATE,
      'قبض عملة أجنبية - ' || v_currency || ' - ' || COALESCE(v_order.order_number, ''),
      v_debit_account,
      COALESCE(v_terminal.cash_account_code, '1110'),
      v_foreign_amount * v_rate, -- ILS equivalent
      v_currency, 'pos_currency_exchange',
      v_order.order_number, v_payment_method,
      v_idempotency || '-FX',
      v_foreign_amount, -- actual foreign amount (e.g. 100 USD)
      v_rate -- exchange rate used
    );

    -- 3) Exchange rate difference (if any)
    ...
    IF ABS(v_rate_diff) > 0.01 THEN
      INSERT INTO public.transactions (
        ...
        CASE WHEN v_rate_diff > 0 THEN COALESCE(v_terminal.cash_account_code, '1110') ELSE '7200' END,
        CASE WHEN v_rate_diff > 0 THEN '4200' ELSE COALESCE(v_terminal.cash_account_code, '1110') END,
  ...
  ELSE
    -- ======= STANDARD ILS FLOW (single entry) =======
    INSERT INTO public.transactions (
      user_id, transaction_date, description,
      debit_account_code, credit_account_code,
      amount, currency, transaction_type, contact_id,
