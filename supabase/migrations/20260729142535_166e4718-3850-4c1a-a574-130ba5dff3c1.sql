CREATE INDEX IF NOT EXISTS idx_transactions_user_deleted_id
ON public.transactions (user_id, id)
WHERE is_deleted = true;

CREATE OR REPLACE FUNCTION public.get_pos_reports_summary(p_from date, p_to date, p_branch uuid DEFAULT NULL::uuid)
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY DEFINER
