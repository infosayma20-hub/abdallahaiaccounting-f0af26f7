...
ALTER TABLE public.pos_terminals
  ADD COLUMN IF NOT EXISTS discount_account_code TEXT,
  ADD COLUMN IF NOT EXISTS max_manager_discount_percent NUMERIC NOT NULL DEFAULT 50;

COMMENT ON COLUMN public.pos_terminals.discount_account_code IS
  'حساب Contra-Revenue لخصم المبيعات الخاص بالفرع. إذا NULL يتم خصم الإيراد مباشرة دون قيد منفصل.';
...
-- Audit table
CREATE TABLE IF NOT EXISTS public.pos_order_discounts (
  ...
  subtotal_before NUMERIC NOT NULL,
  reason TEXT,
  account_code TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_pos_order_discounts_order ON public.pos_order_discounts(order_id);
CREATE INDEX IF NOT EXISTS idx_pos_order_discounts_user_date ON public.pos_order_discounts(user_id, created_at DESC);

GRANT SELECT, INSERT, UPDATE, DELETE ON public.pos_order_discounts TO authenticated;
GRANT ALL ON public.pos_order_discounts TO service_role;

ALTER TABLE public.pos_order_discounts ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "team can view pos discounts" ON public.pos_order_discounts;
CREATE POLICY "team can view pos discounts" ON public.pos_order_discounts
  FOR SELECT USING (public.is_team_member(auth.uid(), user_id));

DROP POLICY IF EXISTS "team can insert pos discounts" ON public.pos_order_discounts;
CREATE POLICY "team can insert pos discounts" ON public.pos_order_discounts
  FOR INSERT WITH CHECK (public.is_team_member(auth.uid(), user_id));

-- ============================================================
-- Phase 2: complete_pos_order — discount-aware accounting
-- ============================================================
...
BEGIN
  ...
  -- ===== NEW: order-level discount handling =====
  v_lines_subtotal := v_net_total;
  v_discount_amt   := GREATEST(COALESCE(v_order.discount_amount, 0), 0);
  -- Cap discount to lines_subtotal so we never post negative revenue
  ...
  IF v_discount_amt > 0 AND (v_discount_acc IS NULL OR v_discount_acc = '') THEN
    -- Fallback: reduce revenue directly (keeps books balanced when no contra account is configured)
    v_net_total := v_lines_subtotal - v_discount_amt;
  END IF;
  -- If discount_acc is set: v_net_total remains = lines_subtotal,
  -- and we'll post a separate Dr discount_acc line after the tender loop.
  ...
  -- ===== NEW: post the contra-revenue (sales discount) entry =====
  -- Dr discount_acc, Cr revenue_acc — balances revenue back to lines_subtotal
