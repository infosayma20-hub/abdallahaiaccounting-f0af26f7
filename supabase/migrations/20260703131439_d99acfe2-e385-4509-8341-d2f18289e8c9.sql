...
CREATE OR REPLACE FUNCTION public.ensure_employee_sub_account(
  p_data_owner uuid,
  p_employee_id uuid
...
BEGIN
  ...
  -- 2) If a matching sub-account already exists for this tenant, return it.
  ...
  -- 3) Ensure parent 2180 exists for this tenant
  ...
  -- 4) Compute next child code: 21801..21899, then 218100+
  SELECT COALESCE(
  ...
  LOOP
    IF v_next_num < 100 THEN
      v_new_code := '218' || lpad(v_next_num::text, 2, '0');
    ELSE
      v_new_code := '218' || v_next_num::text;
    END IF;
    -- ensure uniqueness within tenant
    IF NOT EXISTS (SELECT 1 FROM public.accounts WHERE user_id = p_data_owner AND account_code = v_new_code) THEN
      EXIT;
    END IF;
    v_next_num := v_next_num + 1;
  END LOOP;

  -- 5) Create the sub-account
  INSERT INTO public.accounts (
    user_id, account_code, account_name, account_type, parent_code,
    is_active, is_system_protected, nature
  ) VALUES (
...
GRANT EXECUTE ON FUNCTION public.ensure_employee_sub_account(uuid, uuid) TO authenticated, service_role;
