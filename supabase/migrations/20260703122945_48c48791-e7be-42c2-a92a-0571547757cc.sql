...
BEGIN
  ---------------------------------------------------------------
  -- STEP 1: Fix contacts linked to wrong root (supplier→113, customer→211)
  ---------------------------------------------------------------
  FOR v_contact IN
  ...
  LOOP
    ...
    -- Look for an existing sub-account under the correct root (by name or by contact_id)
    ...
    IF v_target_code IS NULL THEN
      -- Need to create one via resolve_postable_account (only if not dry-run)
    ...
    -- Move all transactions on wrong AR account to correct AP account
    SELECT COUNT(*), COALESCE(SUM(amount),0) INTO v_moved, v_amount
    FROM public.transactions
    WHERE user_id = p_user_id AND NOT is_deleted
      AND (debit_account_code = v_ar_code OR credit_account_code = v_ar_code);

    IF NOT p_dry_run THEN
      -- Update debit side
      UPDATE public.transactions
      SET debit_account_code = v_target_code
      WHERE user_id = p_user_id AND debit_account_code = v_ar_code AND NOT is_deleted;

      -- Update credit side
      UPDATE public.transactions
      SET credit_account_code = v_target_code
      WHERE user_id = p_user_id AND credit_account_code = v_ar_code AND NOT is_deleted;

      -- Backfill contact_id on transactions matched by description or old account
      UPDATE public.transactions
      ...
      -- Relink contact to the correct account
      UPDATE public.contacts
      SET linked_account_code = v_target_code
      WHERE id = v_contact.id AND user_id = p_user_id;

      -- Ensure account has contact_id set
      UPDATE public.accounts
      SET contact_id = v_contact.id
      WHERE user_id = p_user_id AND account_code = v_target_code AND contact_id IS NULL;

      -- Deactivate the wrong AR account (keeping it for audit history)
      UPDATE public.accounts
      SET is_active = false,
          notes = COALESCE(notes,'') || E'\n[REPAIRED ' || v_batch || '] merged into ' || v_target_code
      WHERE user_id = p_user_id AND account_code = v_ar_code AND NOT is_system_protected;
      ...
      VALUES (v_batch, 'contact_relink', v_contact.id,
        ...
        jsonb_build_object('linked_account_code', v_target_code, 'moved_transactions', v_moved, 'total_amount', v_amount),
  ...
  ---------------------------------------------------------------
  -- STEP 2: Reroute transactions posted directly to parent root (1130/2110/2180)
  --         Only when contact_id is present (safe reroute).
  ---------------------------------------------------------------
  FOR v_tx IN
  ...
  ---------------------------------------------------------------
  -- STEP 3: Report transactions on parent root WITHOUT contact_id (needs manual review)
  ---------------------------------------------------------------
  RETURN QUERY
  SELECT
