...
BEGIN
  ...
  -- Create the unified voucher row (so listings & "open voucher" work)
  ...
  INSERT INTO public.vouchers(
    ...
    v_total * COALESCE(p_exchange_rate, 1),
    COALESCE(p_description, (CASE WHEN p_kind='receipt' THEN 'سند قبض مختلط - ' ELSE 'سند صرف مختلط - ' END) || COALESCE(p_contact_name,'')),
    p_notes,
  ...
  -- Now insert cheques with voucher_id linkage
  IF v_cheques_total > 0 THEN
