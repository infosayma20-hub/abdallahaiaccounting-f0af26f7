CREATE INDEX IF NOT EXISTS idx_pos_orders_user_bdate_branch
  ON public.pos_orders (user_id, business_date, branch_id);

CREATE INDEX IF NOT EXISTS idx_call_center_orders_pos_order
  ON public.call_center_orders (pos_order_id, created_at DESC)
  WHERE pos_order_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_pos_payments_order_method
...
filtered AS (
  SELECT * FROM base
  WHERE app IS NOT NULL AND upper(app) <> 'KIOSK'
),
pay AS (
  SELECT
    f.id,
...
REVOKE ALL ON FUNCTION public.get_delivery_apps_report(date, date, uuid) FROM PUBLIC, anon;
