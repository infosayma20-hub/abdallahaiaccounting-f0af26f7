...
BEGIN
  ...
  -- Pre-compute totals + check for unknown cost
  ...
  -- Items + stock movements + decrement
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    INSERT INTO public.invoice_items (
      ...
      v_invoice_id, (v_item->>'product_id')::uuid, v_item->>'name',
    ...
    VALUES (p_user_id, (v_item->>'product_id')::uuid, 'sale'::movement_type, v_qty, p_warehouse_id, 'REP-SALE ' || v_invoice_no);
  ...
  IF p_payment_method = 'cash' THEN
    v_recv_rpc := public.create_receipt_with_entry(
      p_user_id => p_user_id, p_contact_id => NULL,
      p_contact_name => COALESCE(p_contact_name, 'بيع نقدي - مندوب'),
      p_amount => v_total, p_payment_method => 'نقدي',
      p_description => 'Rep cash sale ' || v_invoice_no, p_currency => 'شيكل',
      p_idempotency_key => p_idempotency_key || '-RCV',
      p_voucher_date => CURRENT_DATE, p_exchange_rate => NULL,
      p_reference => v_invoice_no, p_cash_account_code => NULL,
      p_contact_account_code => NULL, p_notes => 'Auto receipt from REP cash sale',
      p_employee_id => NULL, p_workshop_id => NULL, p_allocations => NULL
    );
  END IF;

  RETURN jsonb_build_object(
    'success', true, 'invoice_id', v_invoice_id, 'invoice_number', v_invoice_no,
    'total', v_total, 'total_cost', v_total_cost, 'total_profit', v_total_profit,
    'has_unknown_cost', v_has_unknown_cost,
    'invoice_rpc', v_inv_rpc, 'receipt_rpc', v_recv_rpc
  );
...
GRANT EXECUTE ON FUNCTION public.create_rep_sale_atomic(UUID, UUID, UUID, UUID, UUID, TEXT, TEXT, JSONB, TEXT, TEXT) TO authenticated, service_role;