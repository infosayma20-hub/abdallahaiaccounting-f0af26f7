-- 1) Guard: no invoice-scoped stock movement without a line reference
CREATE OR REPLACE FUNCTION public.guard_invoice_stock_movement_line()
RETURNS trigger
LANGUAGE plpgsql
SET search_path TO 'public'
AS $$
BEGIN
  IF NEW.reference_type = 'invoice' AND NEW.reference_line_id IS NULL THEN
    RAISE EXCEPTION 'invoice stock movements must carry reference_line_id (single writer: invoice_items trigger)';
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_guard_invoice_stock_movement_line ON public.stock_movements;
CREATE TRIGGER trg_guard_invoice_stock_movement_line
BEFORE INSERT ON public.stock_movements
FOR EACH ROW EXECUTE FUNCTION public.guard_invoice_stock_movement_line();

-- 2) Drop unused legacy overloads (app calls the 13-arg version only)
DROP FUNCTION IF EXISTS public.create_rep_sale_atomic(uuid,uuid,uuid,uuid,text,text,text,text,jsonb);
DROP FUNCTION IF EXISTS public.create_rep_sale_atomic(uuid,uuid,uuid,uuid,uuid,text,text,jsonb,text,text);

-- 3) Rep sale: single stock writer
CREATE OR REPLACE FUNCTION public.create_rep_sale_atomic(p_user_id uuid, p_sales_rep_id uuid, p_warehouse_id uuid, p_van_day_id uuid, p_contact_id uuid, p_contact_name text, p_payment_method text, p_items jsonb, p_idempotency_key text, p_invoice_number text DEFAULT NULL::text, p_discount_type text DEFAULT NULL::text, p_discount_value numeric DEFAULT 0, p_cash_account_code text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  IF p_payment_method = 'cash' THEN
    v_cash_acct := COALESCE(NULLIF(TRIM(p_cash_account_code), ''), '1115');

    INSERT INTO public.transactions (
      user_id, transaction_date, description,
      debit_account_code, credit_account_code,
      amount, currency, transaction_type, reference,
      payment_method, idempotency_key, contact_id, sales_rep_id
    ) VALUES (
      p_user_id, CURRENT_DATE,
      'Rep sale ' || p_idempotency_key,
      v_cash_acct, '4110',
      v_total, 'شيكل', 'sale', p_idempotency_key,
      v_pm_arabic, p_idempotency_key, NULL, p_sales_rep_id
  ...
  ELSE
    ...
    UPDATE public.transactions
       SET sales_rep_id = COALESCE(sales_rep_id, p_sales_rep_id)
     WHERE id = v_tx_id;
  END IF;
  ...
  -- Items only. Stock movement + product qty are handled exclusively by
  -- trg_invoice_item_stock_sync -> trg_sync_product_qty (single writer).
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    IF NOT v_disable_stock AND p_warehouse_id IS NOT NULL THEN
      UPDATE public.stock_movements
         SET warehouse_id = COALESCE(warehouse_id, p_warehouse_id)
       WHERE reference_type = 'invoice' AND reference_line_id = v_line_id;
    END IF;
  END LOOP;
...
GRANT EXECUTE ON FUNCTION public.create_rep_sale_atomic(uuid,uuid,uuid,uuid,uuid,text,text,jsonb,text,text,text,numeric,text) TO authenticated, service_role;