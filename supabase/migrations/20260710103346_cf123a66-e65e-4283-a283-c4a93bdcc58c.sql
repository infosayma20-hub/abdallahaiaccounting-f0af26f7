-- Phase A: eliminate double stock deduction in POS.
-- Since 2026-07-07 trigger trg_sync_product_qty auto-syncs products.quantity
-- from stock_movements. Both complete_pos_order and process_pos_return also
-- updated products.quantity directly, causing a double hit for any tenant
-- with pos_disable_stock_deduction=false.
--
-- Safety context verified before writing this migration:
--   * Malaki (شركة مطاعم الدجاج الملكي): pos_disable_stock_deduction=true → block never runs.
--   * All other tenants: 0 paid POS orders since 2026-07-07 → no live stock rows to reconcile.
-- Effect on live data: none. Effect on future sales: single, correct movement.
--
-- Change: remove the direct UPDATE public.products from both functions.
-- _pos_sync_stock_movements (idempotent via ON CONFLICT DO NOTHING) becomes
-- the single source of truth; the trigger propagates to products.quantity.
-- void_pos_order (missing stock reversal on paid voids) is intentionally NOT
-- touched here — separate, larger design change.
...
BEGIN
  ...
  -- Stock: single source of truth via _pos_sync_stock_movements.
  -- Direct UPDATE removed (was causing double deduction with trigger trg_sync_product_qty).
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- Direct UPDATE removed: trg_sync_product_qty on stock_movements now handles
    -- quantity sync. Only _pos_sync_stock_movements below (idempotent).
  END LOOP;
