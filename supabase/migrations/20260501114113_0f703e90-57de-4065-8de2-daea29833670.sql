-- Phase 7.2: link rep-sale invoices to their journal transaction.
-- Adds a single UPDATE after invoice resolution to set invoices.linked_transaction_id.
-- No financial logic, no UI, no account changes.
...
BEGIN
  ...
  IF v_invoice_id IS NULL THEN
    INSERT INTO public.invoices (user_id, warehouse_id, contact_id, invoice_number, invoice_type, status, payment_method, total_amount, linked_transaction_id)
  ...
  ELSE
    -- Phase 7.2: ensure the invoice points to its journal entry (fixes invoice_no_link drift)
    UPDATE public.invoices
       SET linked_transaction_id = v_tx_id
     WHERE id = v_invoice_id
       AND linked_transaction_id IS DISTINCT FROM v_tx_id
       AND v_tx_id IS NOT NULL;
  END IF;
  ...
  -- Phase 7.1: cash leg posted by create_invoice_with_entry; no extra receipt voucher.
