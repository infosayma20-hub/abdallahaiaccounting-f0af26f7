...
BEGIN
  ...
  IF v_manual THEN
    ...
    ELSE
      ...
      -- الاستراحات اليدوية القابلة للاحتساب ضمن مسبح المغادرات المدفوعة:
      -- غير مدفوعة أصلاً + تُحتسب ضمن السقف + ليست مهمة خارجية.
    ...
    -- السياسة الموحدة: أول 30 دقيقة (يدوي + مبصوم) مدفوعة — هنا اليدوي فقط
    -- لأن اليوم المعدّل يدوياً لا يعتمد على فجوات البصمات.
  ...
  ELSE
    ...
    -- السياسة الموحدة: أول 30 دقيقة من مجموع (المغادرات المبصومة +
    -- الاستراحات اليدوية القابلة للاحتساب) مدفوعة ضمن ساعات الدوام.
    IF v_paid_enabled AND p_date >= v_policy_from THEN
