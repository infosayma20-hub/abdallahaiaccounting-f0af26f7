...
-- ============================================================
-- 1) Fix change_pos_payment_method: use v_session.cash_box_id,
--    remove silent exception swallow, assert GL was updated.
-- ============================================================
...
BEGIN
  ...
  IF p_new_method = 'mixed' THEN
    ...
    FOR v_split_row IN SELECT * FROM jsonb_array_elements(p_split_payments) LOOP
      ...
      VALUES (v_company_id, p_order_id, v_split_row->>'method',
        (v_split_row->>'amount')::numeric, (v_split_row->>'amount')::numeric, 0,
  ...
  BEGIN
    ...
    -- FIX: cash_box_id lives on pos_sessions, not pos_orders
    ...
    IF p_new_method <> 'mixed' THEN
      ...
      IF v_gl_rows_updated = 0 THEN
        RAISE EXCEPTION 'GL_SYNC_NO_ROWS' USING ERRCODE='P0001',
          HINT = format('لم يتم العثور على قيود محاسبية للفاتورة %s لتحديثها', v_order.order_number);
      END IF;
    ELSE
      UPDATE public.transactions
    ...
    END IF;
  END;
  -- ======== end GL SYNC (errors propagate = rollback) ========
...
GRANT EXECUTE ON FUNCTION public.change_pos_payment_method(uuid,text,text,uuid,uuid,integer,text,numeric,uuid,jsonb)
  TO authenticated, service_role;

-- ============================================================
-- 2) Repair: resync GL for a single POS order to match its
--    current pos_payments state (fixes historical mismatches).
-- ============================================================
CREATE OR REPLACE FUNCTION public.resync_pos_payment_gl(p_order_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
DECLARE
  v_order pos_orders%ROWTYPE; v_session pos_sessions%ROWTYPE;
  v_caller_owner uuid; v_company_id uuid;
  v_terminal record; v_box_gl_code text; v_card_bank_gl text := '1120';
  v_new_rev_acc text := '4100'; v_new_vat_acc text;
  v_pay_rows int; v_pay record; v_new_debit text;
  v_updated int := 0; v_deleted int := 0; v_inserted int := 0;
  v_net_total numeric; v_vat_total numeric;
  v_pay_currency text; v_running_net numeric := 0; v_running_vat numeric := 0;
  v_tender_share numeric; v_tender_net numeric; v_tender_vat numeric;
  v_customer_id uuid; v_idx int := 0; v_emp_debit text;
BEGIN
  SELECT * INTO v_order FROM pos_orders WHERE id = p_order_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'ORDER_NOT_FOUND' USING ERRCODE='P0001'; END IF;
  v_caller_owner := resolve_effective_owner_id(auth.uid());
  ...
  SELECT * INTO v_session FROM pos_sessions WHERE id = v_order.session_id;
  ...
  IF v_session.terminal_id IS NOT NULL THEN
    SELECT * INTO v_terminal FROM public.pos_terminals WHERE id = v_session.terminal_id;
    IF v_terminal.id IS NOT NULL THEN
      SELECT COALESCE(ba.gl_account_code, '1120') INTO v_card_bank_gl
        FROM public.pos_terminals t
  ...
  SELECT credit_account_code INTO v_new_vat_acc
    FROM public.transactions
   WHERE user_id = v_company_id AND reference = v_order.order_number
     AND transaction_type = 'pos_sale_vat' AND COALESCE(is_deleted,false) = false
   LIMIT 1;

  v_customer_id := v_order.customer_id;
  SELECT count(*) INTO v_pay_rows FROM pos_payments WHERE order_id = p_order_id;
  IF v_pay_rows = 0 THEN RAISE EXCEPTION 'NO_PAYMENTS_FOUND' USING ERRCODE='P0001'; END IF;

  v_net_total := COALESCE(v_order.subtotal, v_order.total, 0) - COALESCE(v_order.discount_amount, 0);
  v_vat_total := COALESCE(v_order.tax_amount, 0);
  IF v_net_total < 0 THEN v_net_total := 0; END IF;

  IF v_pay_rows = 1 THEN
    SELECT * INTO v_pay FROM pos_payments WHERE order_id = p_order_id LIMIT 1;
    v_pay_currency := COALESCE(upper(v_pay.currency),'ILS');

    IF v_pay.payment_method = 'credit' THEN v_new_debit := '1130';
    ELSIF v_pay.payment_method = 'card' THEN v_new_debit := v_card_bank_gl;
    ELSIF v_pay.payment_method = 'employee_account' THEN
      SELECT COALESCE(linked_account_code, '2180') INTO v_emp_debit
        FROM public.employees e
        JOIN public.employee_financial_movements m ON m.employee_id = e.id
       WHERE m.source_type='pos_meal' AND m.source_id = p_order_id AND m.status='approved'
         AND m.movement_type='debit'
       ORDER BY m.created_at DESC LIMIT 1;
      v_new_debit := COALESCE(v_emp_debit, '2180');
    ELSE
      v_new_debit := public._pos_resolve_cash_gl(v_session.cash_box_id, v_pay_currency, v_box_gl_code);
    END IF;

    UPDATE public.transactions
       SET debit_account_code = v_new_debit, payment_method = v_pay.payment_method, updated_at = now(),
           notes = COALESCE(notes,'') || format(
             E'\n[gl-resync %s] method->%s cur->%s (repair)',
             to_char(now(),'YYYY-MM-DD HH24:MI'), v_pay.payment_method, v_pay_currency)
     WHERE user_id = v_company_id AND reference = v_order.order_number
       AND transaction_type IN ('pos_sale','pos_sale_vat')
       AND COALESCE(is_deleted,false) = false;
    GET DIAGNOSTICS v_updated = ROW_COUNT;
  ELSE
    -- multi/split: soft-delete old GL, re-post per pos_payments
    UPDATE public.transactions
       SET is_deleted = true, updated_at = now(),
           notes = COALESCE(notes,'') || format(
             E'\n[gl-resync %s] soft-deleted for repair',
             to_char(now(),'YYYY-MM-DD HH24:MI'))
     WHERE user_id = v_company_id AND reference = v_order.order_number
       AND transaction_type IN ('pos_sale','pos_sale_vat')
       AND COALESCE(is_deleted,false) = false;
    ...
    FOR v_pay IN SELECT * FROM pos_payments WHERE order_id = p_order_id ORDER BY created_at LOOP
      v_idx := v_idx + 1;
      v_pay_currency := COALESCE(upper(v_pay.currency),'ILS');
      IF v_pay.payment_method = 'credit' THEN v_new_debit := '1130';
      ELSIF v_pay.payment_method = 'card' THEN v_new_debit := v_card_bank_gl;
      ELSIF v_pay.payment_method = 'employee_account' THEN
        v_new_debit := '2180';
      ELSE v_new_debit := public._pos_resolve_cash_gl(v_session.cash_box_id, v_pay_currency, v_box_gl_code);
      END IF;

      IF v_idx = v_pay_rows THEN
        v_tender_net := GREATEST(v_net_total - v_running_net, 0);
        v_tender_vat := GREATEST(v_vat_total - v_running_vat, 0);
      ELSIF COALESCE(v_order.total,0) > 0 THEN
        v_tender_share := v_pay.amount / v_order.total;
        v_tender_net := ROUND(v_net_total * v_tender_share, 2);
        v_tender_vat := ROUND(v_vat_total * v_tender_share, 2);
        v_running_net := v_running_net + v_tender_net;
        v_running_vat := v_running_vat + v_tender_vat;
      ...
      IF v_tender_net > 0 THEN
        ...
        VALUES (v_company_id, CURRENT_DATE,
          'مبيعات نقطة البيع - ' || COALESCE(v_order.order_number,'') || ' (' || v_pay.payment_method || ')',
          v_new_debit, v_new_rev_acc, v_tender_net,
          CASE v_pay_currency WHEN 'USD' THEN 'دولار' WHEN 'JOD' THEN 'دينار' WHEN 'EUR' THEN 'يورو' ELSE 'شيكل' END,
          'pos_sale', v_customer_id, v_order.order_number, v_pay.payment_method,
          'POS-ORDER-' || v_order.id::text || '-RESYNC-T' || v_idx || '-' || floor(extract(epoch from now()))::text);
      ...
      IF v_tender_vat > 0 AND v_new_vat_acc IS NOT NULL THEN
        INSERT INTO public.transactions(
          ...
          amount, currency, transaction_type, contact_id, reference, payment_method, idempotency_key)
        VALUES (v_company_id, CURRENT_DATE,
          'ضريبة قيمة مضافة (مخرجات) - POS ' || COALESCE(v_order.order_number,'') || ' (' || v_pay.payment_method || ')',
          v_new_debit, v_new_vat_acc, v_tender_vat,
          CASE v_pay_currency WHEN 'USD' THEN 'دولار' WHEN 'JOD' THEN 'دينار' WHEN 'EUR' THEN 'يورو' ELSE 'شيكل' END,
          'pos_sale_vat', v_customer_id, v_order.order_number, v_pay.payment_method,
          'POS-ORDER-' || v_order.id::text || '-RESYNC-VAT-T' || v_idx || '-' || floor(extract(epoch from now()))::text);
        v_inserted := v_inserted + 1;
      END IF;
    END LOOP;
  END IF;
  ...
  INSERT INTO pos_sensitive_actions_log(
    company_id, action, pos_user_id, manager_user_id,
    session_id, invoice_id, notes, metadata)
  VALUES (v_company_id, 'resync_pos_payment_gl', NULL, auth.uid(),
    v_order.session_id, v_order.id, 'إعادة مزامنة القيود المحاسبية لمطابقة الدفعات الحالية',
    jsonb_build_object('gl_rows_updated', v_updated, 'gl_rows_deleted', v_deleted,
                       'gl_rows_inserted', v_inserted));

  RETURN jsonb_build_object('ok', true, 'order_id', v_order.id,
    'gl_rows_updated', v_updated, 'gl_rows_deleted', v_deleted, 'gl_rows_inserted', v_inserted);
END;
$function$;

GRANT EXECUTE ON FUNCTION public.resync_pos_payment_gl(uuid) TO authenticated, service_role;

-- ============================================================
-- 3) Bulk repair for a session (e.g. Emran's shift).
-- ============================================================
CREATE OR REPLACE FUNCTION public.resync_pos_payments_gl_for_session(p_session_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
DECLARE
  v_caller_owner uuid; v_session pos_sessions%ROWTYPE;
  v_order_id uuid; v_repaired int := 0; v_skipped int := 0; v_res jsonb;
BEGIN
  ...
  v_caller_owner := resolve_effective_owner_id(auth.uid());
  IF v_caller_owner IS NULL OR v_session.user_id <> v_caller_owner THEN
    RAISE EXCEPTION 'ACCESS_DENIED' USING ERRCODE='P0001'; END IF;

  FOR v_order_id IN
    SELECT DISTINCT po.id
      FROM pos_orders po
      JOIN pos_payments pp ON pp.order_id = po.id
      LEFT JOIN transactions t ON t.reference = po.order_number
                               AND t.transaction_type='pos_sale'
                               AND COALESCE(t.is_deleted,false)=false
                               AND t.user_id = po.user_id
     WHERE po.session_id = p_session_id
       AND po.state = 'paid' AND COALESCE(po.is_return,false)=false
       AND po.cancelled_at IS NULL
       AND (pp.payment_edited_at IS NOT NULL
            OR pp.payment_method <> COALESCE(t.payment_method,''))
  LOOP
    BEGIN
      v_res := public.resync_pos_payment_gl(v_order_id);
  ...
  RETURN jsonb_build_object('ok', true, 'session_id', p_session_id,
    'repaired', v_repaired, 'skipped', v_skipped);
END;
$function$;

GRANT EXECUTE ON FUNCTION public.resync_pos_payments_gl_for_session(uuid) TO authenticated, service_role;
