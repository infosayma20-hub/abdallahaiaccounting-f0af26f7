...
BEGIN
  ...
  v_receipt_ref := COALESCE(v_order.order_number, p_order_id::text);
  ...
  SELECT string_agg(DISTINCT payment_method, ','),
         min(currency), min(exchange_rate), sum(amount),
         (array_agg(payment_method ORDER BY created_at))[1]
    INTO v_old_methods, v_old_currency, v_old_rate, v_old_amount, v_old_primary_method
    FROM pos_payments WHERE order_id = p_order_id;

  v_new_amount := COALESCE(v_order.total, v_old_amount);

  IF v_old_primary_method = 'employee_account' THEN
  ...
  IF p_new_method = 'mixed' THEN
    FOR v_split_row IN SELECT * FROM jsonb_array_elements(p_split_payments) LOOP
      INSERT INTO pos_payments(order_id, payment_method, amount, currency, exchange_rate, tendered, user_id, created_at, change_amount, change_currency)
      VALUES (
        p_order_id,
        v_split_row->>'method',
    ...
    END LOOP;
  ELSE
    INSERT INTO pos_payments(order_id, payment_method, amount, currency, exchange_rate, tendered, user_id, created_at)
    VALUES (
  ...
  UPDATE pos_orders
     SET currency = upper(COALESCE(p_new_currency, v_order.currency, 'ILS')),
         updated_at = now()
   WHERE id = p_order_id;
  ...
  BEGIN
    ...
    IF v_je_id IS NOT NULL THEN
      ...
      v_target_debit_accounts := ARRAY[
        '1110','1112','1113','1114','1120','1130','1131','1132','1133','1134','1135','1150','1160'
      ];
      ...
      IF p_new_method <> 'mixed' THEN
        ...
        v_je_rate     := COALESCE(p_new_exchange_rate, 1);
        ...
        INSERT INTO public.journal_entry_lines(
          ...
          format('POS %s — %s', v_receipt_ref, p_new_method),
          v_je_amount, 0,
          v_je_foreign, 0,
      ...
      ELSE
        FOR v_tender IN SELECT * FROM jsonb_array_elements(p_split_payments) LOOP
          ...
          INSERT INTO public.journal_entry_lines(
            ...
            format('POS %s — %s (mixed)', v_receipt_ref, v_tender->>'method'),
            v_line_local, 0,
            v_line_native, 0,
  ...
  IF p_new_method = 'employee_account' THEN
    ...
    INSERT INTO public.employee_financial_movements(
      ...
      format('POS %s — %s', v_receipt_ref, v_emp_name),
      ...
      jsonb_build_object('meal_full', v_meal_full, 'meal_pct', v_meal_pct, 'meal_deducted', v_meal_deducted,
                         'changed_from', v_old_primary_method, 'changed_at', now())
    );
  END IF;
