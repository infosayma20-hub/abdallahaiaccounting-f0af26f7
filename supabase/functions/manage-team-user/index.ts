...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "update_profile") {
      const { target_user_id, display_name } = body;
      const name = String(display_name ?? "").trim();
      if (!target_user_id || name.length < 2) {
        return new Response(JSON.stringify({ error: "الاسم مطلوب (حرفان على الأقل)" }), {
          status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }

      // Tenant check (super_admin bypasses)
      if (!hasSuperAdmin) {
        const [{ data: adminProfile }, { data: targetProfile }] = await Promise.all([
          supabase.from("profiles").select("company_id").eq("user_id", adminUser.id).maybeSingle(),
          supabase.from("profiles").select("user_id, invited_by, company_id").eq("user_id", target_user_id).maybeSingle(),
      ...
      await supabase.from("activity_log").insert({
        user_id: adminUser.id, actor_id: adminUser.id,
        actor_name: adminUser.user_metadata?.full_name || adminUser.email || "",
        action: "update", entity_type: "user", entity_id: target_user_id,
        entity_label: name, details: { display_name: name },
      });
