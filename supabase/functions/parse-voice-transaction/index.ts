...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `أنت محاسب ذكي مدمج في نظام "يونيفاي".
...
## الخطوة 0: كشف نية التعديل أو الحذف
إذا احتوى النص على: غلطت، صحّح، عدّل، بدّل، كان لازم، مش صح، الرقم غلط، اكتبت غلط، غلط بالمبلغ
فإن intent = "edit_transaction"
إذا احتوى النص على: امسح، احذف، ألغِ، ما بدي اياها، غلط كلياً، مش لازم تكون
فإن intent = "delete_transaction"

استخرج:
- action: "edit" أو "delete" أو "cancel"
- target: وصف المعاملة المستهدفة ("آخر معاملة" / "فاتورة أحمد" / رقم الفاتورة / مبلغ)
- correction (فقط للتعديل):
  - field: "amount" / "party" / "date" / "payment_method"
  - old_value: القيمة القديمة (إن ذُكرت)
  - new_value: القيمة الجديدة (إن ذُكرت)

أرجع:
{
  "type": "edit_transaction",
  "action": "edit" أو "delete",
  "target": "وصف المعاملة",
  "correction": { "field": "", "old_value": "", "new_value": "" },
  "status": "complete"
}

## الخطوة 0A: كشف نية إضافة كيان جديد
...
## الخطوة 0C: كشف الأصول الثابتة
إذا احتوى النص على: سيارة، شاحنة، معدات، آلة، مكينة، أثاث، كمبيوتر، تجهيزات، عقار، أرض، مولد، مكيف، طابعة
وكان السياق شراء (اشتريت، جبت، كلفني):
→ سجّلها كفاتورة مشتريات عادية (النظام سيتعرف عليها كأصل ثابت تلقائياً)
→ أضف في الوصف اسم الأصل

## الخطوة 0D: كشف الرواتب والسلف
"صرّفت راتب أحمد" / "راتب العمال" → سجّل كسند صرف مع وصف "راتب — [الاسم]"
"سلّفت محمد" / "أعطيت سلفة" → سجّل كسند صرف مع وصف "سلفة — [الاسم]"
