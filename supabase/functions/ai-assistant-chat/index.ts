...
serve(async (req) => {
  ...
  try {
    ...
    const pageContextMap: Record<string, string> = {
      ...
      '/cheques': 'صفحة إدارة الشيكات - يمكنك مساعدته في تسجيل شيكات واردة/صادرة وتغيير حالاتها وتحليل استحقاقاتها',
...
## إدارة الشيكات
أنت تدعم نظام شيكات متكامل:
- الشيكات الواردة (من عملاء) والصادرة (لموردين)
- حالات الشيك: مسجل → آجل → مستحق → مودع → محصل (أو مرتجع/ملغي)
- الشيك لا يؤثر على السيولة (الصندوق/البنك) حتى يتم تحصيله فعلياً
- عند تسجيل شيك وارد: مدين شيكات واردة، دائن العملاء
- عند تحصيل شيك وارد: مدين البنك، دائن شيكات واردة
- عند تسجيل شيك صادر: مدين الموردين، دائن شيكات صادرة
