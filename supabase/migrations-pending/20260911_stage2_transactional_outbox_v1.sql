...
-- The only existing object touched is `create_receipt_command_v1`, which is
-- replaced by a superset of itself: identical behaviour when the tenant flag
-- `events.receipt_v1` is OFF (the default for every tenant).
-- =====================================================================
...
CREATE TABLE IF NOT EXISTS public.domain_events_outbox (
  ...
  -- server-resolved context (never client supplied)
  ...
  -- delivery / status metadata (no external delivery is enabled in Stage 2)
...
-- idempotency: one logical event per (owner, type, dedupe key)
...
-- Grants: read-only for app users, full access for the service role only.
...
-- Tenant-scoped read. No INSERT/UPDATE/DELETE policy exists, therefore no
-- ordinary user (anon or authenticated) can write, edit or delete events.
    ...
    OR EXISTS (
      SELECT 1 FROM public.employees e
       WHERE e.user_id = auth.uid()
         AND e.created_by = public.domain_events_outbox.owner_id
    )
    OR public.has_role(auth.uid(), 'super_admin')
  );

-- ---------------------------------------------------------------------
-- 2. updated_at trigger (uses the existing shared function)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 3. Feature flag resolver (tenant scoped, default OFF)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 4. Event emitter — always called INSIDE the caller's transaction
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  IF v_event_id IS NULL THEN
    -- replay of the same logical fact: return the existing event id
...
-- ---------------------------------------------------------------------
-- 5. Observability view (read-only, tenant scoped through the base table RLS)
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 6. Dispatcher — TEST ONLY, no external delivery.
--    It merely marks events as processed; it performs no HTTP call, no
--    webhook, no email, no third-party integration.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 7. Receipt Command V1 — superset, event emitted in the SAME transaction
--    Behaviour with `events.receipt_v1` OFF is byte-for-byte the current one.
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  IF COALESCE((v_rpc->>'success')::boolean, false) IS NOT TRUE THEN
    ...
    -- NO event is emitted for a failed command.
    RETURN jsonb_build_object(
      'version', 1,
      'status', 'failed',
  ...
  -- 9b. STAGE 2 — committed domain fact, SAME transaction, flag gated.
  -- The receipt is already complete and authoritative at this point; the
  -- event carries no financial responsibility and no sensitive values.
