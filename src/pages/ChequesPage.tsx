...
const ChequesPage = () => {
  ...
  const fetchCheques = useCallback(async () => {
    if (!user) return;
    setLoading(true);
    const { data, error } = await supabase.from('cheques').select('*').eq('user_id', ownerId).order('cheque_date', { ascending: false });
  ...
  const handleQuickAddContact = async (name: string) => {
    ...
    try {
      ...
      const { error } = await supabase.from('contacts').insert({ user_id: ownerId, contact_name: name.trim(), contact_type: contactType });
    ...
    try {
      for (const row of newCheques) {
        ...
        await supabase.from('cheque_status_history').insert({
          cheque_id: chequeId, user_id: ownerId,
  ...
  const handleAction = async (data: ActionFormData) => {
    ...
    try {
      ...
      await supabase.from('cheque_status_history').insert({
        cheque_id: cheque.id, user_id: ownerId,
