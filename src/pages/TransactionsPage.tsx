  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/airtable-update-transaction`,
        {
          method: "POST",
          headers: {
    ...
    <div className="px-4 pt-6 space-y-4" dir="rtl">
      ...
      {!loading && !error && (
        <div className="space-y-2">
          {transactions.map((tx) => (
            <Card key={tx.id} className="border-0 shadow-sm cursor-pointer hover:shadow-md transition-shadow" onClick={() => openEdit(tx)}>
