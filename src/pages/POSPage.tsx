...
const POSPage = () => {
  ...
  // ──────────────────────────────────────────────────────────────────
  // PRE-STAGING SYSTEM — performance optimization, NO behavior change
  // ──────────────────────────────────────────────────────────────────
  // When the payment modal opens we silently INSERT pos_orders (state=draft)
  // and pos_order_lines in the background. When the cashier clicks "تأكيد"
  // handleCompleteOrder reuses that draft and only fires complete_pos_order.
  // If the cashier cancels (closes modal, edits cart), the staged order is
  // either discarded (DELETE) or rebuilt automatically on next stage.
  //
  // Eligibility (KEEP NARROW for safety): online + no table + no call-center
  // + no employee_account + no split mode + cart not empty. All other paths
  // run the original code path unchanged.
  ...
  // Stable hash of everything that goes into the INSERT pos_orders + lines.
  // If ANY of these change after staging we discard the stage and re-stage
  // (or fall back to the original sync path inside handleCompleteOrder).
  const computeStageHash = useCallback((): string => {
    try {
      const cartSig = cart.map((i) => ({
        ...
        t: i.total,
        no: i.note || "",
        m: (i.modifiers || []).map((m: any) => `${m.group_id || ""}:${m.option_id || ""}:${m.option_name || ""}:${m.extra_price || 0}`).join("|"),
      }));
      const sig = {
        c: cartSig,
        cn: customerName || "",
  ...
  // Discard the currently-staged order (fire-and-forget DELETE). Used when
  // the cashier closes the payment modal without paying, or when the cart
  // changes mid-modal and we need to re-stage from scratch.
  const discardStagedOrder = useCallback(() => {
    ...
    // Only delete if still draft — never touch a paid order.
    supabase
      .from("pos_orders")
      .delete()
      .eq("id", id)
  ...
  // The actual background staging routine. Safe to call repeatedly — it
  // no-ops while another stage is in flight, and rebuilds itself when the
  // cart hash changes.
  const stageOrderInBackground = useCallback(async () => {
    // Eligibility guards — keep narrow on purpose.
    if (!userId || !session || !company || !dataOwnerId) return;
    if (cart.length === 0) return;
    if (!offlineMode.isOnline) return;             // offline path owns its own flow
    if (activeOrder.tableId) return;               // table-draft logic is its own beast
    if (activeOrder.callCenterOrderId) return;     // needs target_branch validation first
    if (paymentMethod === "employee_account") return; // sub-account creation has side effects
    if (splitMode) return;                         // split has its own validation
    if (stagingInFlightRef.current) return;

    const hash = computeStageHash();
    if (!hash) return;
    // Already staged for this exact cart? Nothing to do.
    if (stagedOrderIdRef.current && stagedHashRef.current === hash) return;

    // Cart changed → discard previous stage and rebuild.
    ...
    try {
      ...
      // ALWAYS persist the employee identity in order_note (same shape as
      // handleCompleteOrder builds it, kept consistent for reports).
      ...
      // Verify cart hasn't drifted while we were inserting. If it did, the
      // stage is stale — discard and let the next stage call (or the sync
      // fallback in handleCompleteOrder) take over.
      const finalHash = computeStageHash();
      if (finalHash !== hash) {
        // Cart changed during staging — drop this stage.
        supabase
          .from("pos_orders")
          .delete()
          .eq("id", order.id)
          .eq("state", "draft" as any)
          .then(({ error }) => {
            if (error) console.warn("[POS stage] stale stage cleanup failed:", error);
          });
        return;
      }
      ...
      // Staging failed — handleCompleteOrder will fall back to its original
      // synchronous path. The cashier never sees this error.
  ...
  // Keep the ref in sync so openPaymentModal can call it without depending
  // on the function identity (avoids re-renders / dependency churn).
  useEffect(() => {
    stageOrderInBackgroundRef.current = stageOrderInBackground;
  }, [stageOrderInBackground]);

  // If the cart changes while the payment modal is open, re-stage so the
  // hash stays valid.
  useEffect(() => {
