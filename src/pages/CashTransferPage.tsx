...
const CashTransferPage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [bRes, tRes, txRes] = await Promise.all([
      ...
      supabase.from("transactions").select("amount, debit_account_code, credit_account_code").eq("user_id", user.id).eq("is_deleted", false),
    ]);
    ...
    for (const code of codes) {
      ...
      (txRes.data || []).forEach((tx: any) => {
        ...
        if (tx.debit_account_code === code) bal += amt;
        if (tx.credit_account_code === code) bal -= amt;
      });
      bals[code] = bal;
    }
    setBalances(bals);
  ...
  const fromBalance = fromBox ? (balances[fromBox.gl_account_code] || 0) : 0;
  const toBalance = toBox ? (balances[toBox.gl_account_code] || 0) : 0;
  const amountNum = Number(amount) || 0;
  const exceeds = amountNum > fromBalance && fromBalance > 0;
  ...
  useEffect(() => {
    if (fromBox && toBox) {
      setDescription(`تحويل نقدي: ${fromBox.name} → ${toBox.name} — ${transferDate}`);
    }
  }, [fromBox, toBox, transferDate]);
  ...
  const handleTransfer = async () => {
    ...
    if (!fromBox.gl_account_code || !toBox.gl_account_code) { toast({ title: "خطأ", description: "أحد الصناديق غير مرتبط بحساب محاسبي", variant: "destructive" }); return; }
    ...
    const { data: tx, error: txErr } = await supabase.from("transactions").insert({
      ...
      amount: amountNum,
      currency: fromBox.currency === "ILS" ? "شيكل" : fromBox.currency === "USD" ? "دولار" : "دينار",
      ...
      <Card className="overflow-hidden" style={{ borderTop: "4px solid #C9A84C" }}>
        <CardContent className="p-6 md:p-8 space-y-6">
          ...
          <div className="flex items-center justify-center gap-4 py-4">
            <div className={`flex-1 max-w-[200px] p-4 rounded-xl border-2 text-center transition-all ${fromBox ? "border-red-200 bg-red-50/50" : "border-dashed border-muted"}`}>
              ...
              <p className="text-lg font-mono font-bold mt-1 text-red-600">
                {fromBox ? `₪${fmt(fromBalance - amountNum)}` : "—"}
              </p>
              {fromBox && amountNum > 0 && <p className="text-[10px] text-muted-foreground">كان: ₪{fmt(fromBalance)}</p>}
            </div>
            ...
            <div className="flex flex-col items-center gap-1">
              ...
              {amountNum > 0 && <Badge className="text-xs font-mono" style={{ background: "#C9A84C" }}>₪{fmt(amountNum)}</Badge>}
            </div>
            ...
            <div className={`flex-1 max-w-[200px] p-4 rounded-xl border-2 text-center transition-all ${toBox ? "border-emerald-200 bg-emerald-50/50" : "border-dashed border-muted"}`}>
              <p className="text-xs text-muted-foreground mb-1">إلى صندوق</p>
              <p className="text-sm font-bold truncate">{toBox ? `${typeIcons[toBox.type]} ${toBox.name}` : "—"}</p>
              <p className="text-lg font-mono font-bold mt-1 text-emerald-600">
                {toBox ? `₪${fmt(toBalance + amountNum)}` : "—"}
              </p>
              {toBox && amountNum > 0 && <p className="text-[10px] text-muted-foreground">كان: ₪{fmt(toBalance)}</p>}
            </div>
          </div>
          ...
          <div>
            ...
            />
            {exceeds && <p className="text-xs text-red-600 mt-1">⚠️ المبلغ يتجاوز رصيد الصندوق ₪{fmt(fromBalance)}</p>}
            {fromBox && (
              <div className="flex gap-2 mt-2">
                ...
                <button onClick={() => setAmount(String(fromBalance))}
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  <tr className="border-t">
                    ...
                    <td className="p-2.5 font-mono text-xs font-bold">₪{fmt(amountNum)}</td>
                  ...
                  <tr className="border-t">
                    ...
                    <td className="p-2.5"></td>
                    <td className="p-2.5 font-mono text-xs font-bold text-red-600">₪{fmt(amountNum)}</td>
                  </tr>
                  <tr className="border-t bg-muted/30 font-bold text-xs">
                    <td className="p-2.5">الإجمالي</td>
                    <td className="p-2.5 font-mono">₪{fmt(amountNum)}</td>
                    <td className="p-2.5 font-mono text-red-600">₪{fmt(amountNum)}</td>
                  </tr>
                </tbody>
              </table>
