...
import { Camera, Mail, Save, Loader2, LogOut, Trash2, Check, ShieldCheck, KeyRound, ChevronLeft, X } from "lucide-react";
...
import type { SettingsActionGroup } from "@/components/settings/shell";
...
const ProfilePage = () => {
  ...
  const fields = [
    { key: "display_name", label: "الاسم الكامل", placeholder: "أدخل اسمك" },
    { key: "company_name", label: "اسم النشاط / الشركة", placeholder: "مثال: شركة النور للتجارة" },
    { key: "work_field", label: "مجال العمل", placeholder: "مثال: تجارة عامة" },
    { key: "country", label: "الدولة", placeholder: "مثال: فلسطين" },
    { key: "address", label: "العنوان", placeholder: "مثال: رام الله" },
  ] as const;

  const actionGroups: SettingsActionGroup[] = [
    {
      key: "save",
      label: "حفظ",
      items: [
        {
          ...
          disabled: saving,
          loading: saving,
          tooltip: "حفظ بيانات الملف الشخصي",
        },
        {
          key: "cancel",
          label: "إلغاء",
  ...
  return (
    <SettingsShell
      title="الملف الشخصي"
      subtitle="تعديل بيانات الحساب وهوية الشركة"
    ...
    >
      <div className="h-full overflow-auto p-5 space-y-5 max-w-4xl mx-auto" dir="rtl">
        ...
        {/* Basic info */}
        <SettingsSection title="المعلومات الأساسية" description="بيانات الحساب والشركة">
          <div className="space-y-4">
        {fields.map((field) => (
          <div key={field.key} className="flex items-center gap-4">
            <label className="text-[12.5px] font-medium text-foreground w-44 shrink-0 text-left flex items-center gap-2 justify-end">
              {field.label}
            </label>
            <Input
              ...
              className="flex-1 h-10 rounded-md border border-border bg-background text-[13px]"
              dir="rtl"
            />
          </div>
        ))}
        ...
        {/* Brand identity */}
        <SettingsSection title="هوية الشركة البصرية" description="ألوان وثيمات النظام">
          <BrandIdentitySettings />
        </SettingsSection>

        {/* Password shortcut */}
        <SettingsSection title="الأمان" description="إدارة كلمة المرور والتحقق الثنائي">
          <button
            type="button"
            onClick={() => navigate("/settings?section=security")}
            className="w-full flex items-center justify-between gap-3 p-3 rounded-md border border-border bg-card hover:bg-muted/30 transition-colors text-right mb-3"
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[13px] font-semibold text-foreground">إدارة كلمة المرور</p>
                <p className="text-[11.5px] text-muted-foreground">تتم إدارة كلمة المرور من الإعدادات &gt; الأمان</p>
              </div>
            </div>
          ...
          <div className="rounded-md border border-border bg-card p-4 space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-[13px] font-semibold text-foreground">التحقق الثنائي غير مفعّل</p>
                <p className="text-[11.5px] text-muted-foreground">أضف طبقة حماية إضافية لحسابك باستخدام Google Authenticator أو رسالة SMS</p>
              </div>
            </div>
        ...
        {/* Preferences */}
        <SettingsSection title="تفضيلات الإدخال" description="تخصيص سلوك الفواتير وقيود اليومية">
          <FastEntryToggle />
        </SettingsSection>

        {/* Notifications */}
        <SettingsSection title="إعدادات الإشعارات" description="اختر الإشعارات التي تريد استلامها">
          ...
          {[
            ...
            <label key={item.label} className="flex items-start gap-3 p-3 rounded-md border border-border bg-card hover:bg-muted/20 cursor-pointer transition-colors">
