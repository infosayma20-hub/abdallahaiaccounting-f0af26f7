...
import PortalCompensationsTab from "@/pages/portal/PortalCompensationsTab";

/**
 * Read-only compensations workspace for staff who were granted the
 * "التعويضات" permission. Renders the exact same view used in the owner
 * portal (PortalCompensationsTab) so both audiences see an identical screen.
 */
export default function CompensationsViewPage() {
  ...
  return (
    <div dir="rtl" className="min-h-[100dvh] bg-background">
      <header className="sticky top-0 z-10 flex items-center justify-between gap-2 border-b bg-card px-4 py-3">
        <div className="text-sm font-bold">التعويضات</div>
      ...
      {!dataOwnerId ? (
        ...
        <PortalCompensationsTab theme={theme} ownerId={dataOwnerId} />
      )}
    </div>
  );
}
