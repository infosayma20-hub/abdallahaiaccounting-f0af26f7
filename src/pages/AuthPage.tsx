      ...
      if (mode === "forgot") {
        ...
        const { data: signUpData, error } = await supabase.auth.signUp({
          ...
          options: {
            ...
            data: { full_name: displayName, company_name: companyName, signup_method: "email", locale: "ar", currency: "ILS" },
          },
        });
        if (error) throw error;
        ...
        try {
          await supabase.functions.invoke("airtable-create-client", {
            body: { clientName: userId, contactEmail: email, phoneNumber: phone, companyName, address, country, workField },
          });
        } catch (airtableErr) {
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-background" dir="rtl">
      <div className="w-full max-w-sm space-y-6">
        ...
        <div className="text-center space-y-2">
          <div className="w-18 h-18 mx-auto rounded-2xl bg-primary/10 flex items-center justify-center mb-3" style={{ width: 72, height: 72 }}>
            <span className="text-3xl font-bold text-primary">ع</span>
          </div>
          <h1 className="text-2xl font-bold text-foreground">عبدالله AI للمحاسبة</h1>
          <p className="text-sm text-muted-foreground">حوّل كلامك إلى قيود محاسبية فوراً</p>
        </div>

        {/* Biometric Sign In (only for returning users) */}
        ...
        {mode !== "forgot" && (
          <div className="space-y-2">
            <Button
              variant="outline"
              className="w-full gap-3 h-14 text-base border-border hover:bg-muted/50 shadow-sm"
            ...
            >
              ...
              <span className="font-semibold">ابدأ الآن باستخدام Google</span>
            </Button>
            <p className="text-center text-[11px] text-muted-foreground">الدخول السريع الموصى به</p>
          </div>
        )}
        ...
        {mode !== "forgot" && (
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-muted-foreground whitespace-nowrap">أو استخدم بريدك الإلكتروني</span>
            <div className="flex-1 h-px bg-border" />
          </div>
        )}
          ...
          <CardContent className="p-5">
            <form onSubmit={handleEmailAuth} className="space-y-4">
              ...
              <Button type="submit" className="w-full h-12 gap-2 text-base font-semibold rounded-xl shadow-md shadow-primary/20" disabled={loading || (mode === "signup" && !agreedToTerms)}>
                ...
                {mode === "login" ? "دخول" : mode === "signup" ? "إنشاء حساب وبدء الاستخدام" : "إرسال رابط الاستعادة"}
              </Button>
            </form>
          </CardContent>
        </Card>
