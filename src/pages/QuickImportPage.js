...
import { Zap, Users, Wallet, BookOpen, UserCheck, CalendarDays, Truck, FileSpreadsheet, Search, ChevronLeft, Download, Upload, ArrowLeft, } from "lucide-react";
...
const items = [
    {
        id: "payroll",
        title: "تثبيت الرواتب",
        description: "تثبيت رواتب كل الموظفين دفعة واحدة بقيد مركّب من حساب مصروف الرواتب إلى ذمة كل موظف على حدة.",
        icon: Wallet,
    ...
    },
    {
        ...
        description: "استيراد أرصدة العملاء والموردين والحسابات من ملف Excel كأرصدة افتتاحية دفعة واحدة.",
        icon: BookOpen,
    ...
    },
    {
        ...
        description: "استيراد إضافات وخصومات الشهر (بدلات، سلف، ساعات إضافية) قبل احتساب الرواتب.",
        icon: CalendarDays,
    ...
    },
    {
        id: "accounts",
        title: "شجرة الحسابات",
        description: "استيراد أو تعديل شجرة الحسابات من ملف Excel مع الحفاظ على الحسابات المحمية.",
...
const categories = ["الكل", "المحاسبة", "الموارد البشرية", "المشتريات"];
export default function QuickImportPage() {
    ...
    return (<div className="container mx-auto p-4 md:p-6" dir="rtl">
      {/* Header */}
      <div className="mb-4 flex items-center gap-3">
        <button onClick={() => (window.history.length > 2 ? navigate(-1) : navigate("/accounting-center"))} className="rounded-xl p-2 hover:bg-muted" aria-label="رجوع">
        ...
        <aside className="rounded-xl border border-border/60 bg-card">
          <div className="border-b border-border/60 p-3 space-y-2">
            <div className="relative">
              ...
              <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="ابحث عن بند..." className="pr-8 text-sm"/>
            </div>
            <div className="flex flex-wrap gap-1">
              {categories.map((c) => (<button key={c} onClick={() => setCategory(c)} className={`rounded-full px-2.5 py-1 text-[11px] transition-colors ${category === c
          ...
          <ul className="max-h-[560px] overflow-y-auto p-1.5">
            ...
            const IIcon = i.icon;
            return (<li key={i.id}>
                  <button onClick={() => setSelectedId(i.id)} className={`flex w-full items-start gap-3 rounded-lg p-2.5 text-right transition-colors ${active
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {i.badge && (<Badge variant="secondary" className="text-[9px] px-1.5 py-0">{i.badge}</Badge>)}
                      </div>
            ...
            {filtered.length === 0 && (<li className="p-6 text-center text-xs text-muted-foreground">لا توجد نتائج</li>)}
          </ul>
        </aside>
        ...
        <main className="rounded-xl border border-border/60 bg-card">
          ...
          <div className="p-5">
            ...
            <ol className="space-y-2">
              {selected.steps.map((s, i) => (<li key={i} className="flex items-start gap-3">
