...
export default function TaxPurchasesLedger({ ownerId }: Props) {
  const now = new Date();
  ...
  const deductible = filtered.filter(r => r.is_deductible);
  const nonDeductible = filtered.filter(r => !r.is_deductible);
  const totalDeductibleTax = deductible.reduce((s, r) => s + Number(r.tax_amount), 0);
  const totalNonDeductibleTax = nonDeductible.reduce((s, r) => s + Number(r.tax_amount), 0);
  const totalNet = filtered.reduce((s, r) => s + Number(r.net_amount), 0);

  return (
    <Card className="p-6 border border-border">
      <div className="flex items-center justify-between flex-wrap gap-4 mb-4">
        <h3 className="text-lg font-bold text-foreground">كشف ضريبة المشتريات (مدخلات)</h3>
        ...
        <table className="w-full text-sm" style={{ borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "#0D1B2E", color: "#fff" }}>
              ...
              <th className="px-3 py-2.5 text-right text-xs font-medium">اسم المورد</th>
              ...
              <th className="px-3 py-2.5 text-center text-xs font-medium">قابل للخصم</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr key={r.id} className={i % 2 === 0 ? "" : "bg-muted/20"} style={{ borderBottom: "1px solid hsl(var(--border))" }}>
                ...
                <td className="px-3 py-2.5 text-center">
                  {r.is_deductible ? <span className="text-emerald-600">✓ نعم</span> : <span className="text-red-500">✗ لا</span>}
                </td>
              </tr>
            ))}
          </tbody>
          {filtered.length > 0 && (
            <tfoot>
              <tr style={{ background: "#F1F5F9" }} className="font-bold text-sm">
                ...
                <td></td>
                <td className="px-3 py-2.5 text-left tabular-nums text-emerald-600">{fmt(totalDeductibleTax)}</td>
                <td className="px-3 py-2.5 text-center text-xs text-muted-foreground">غير قابل: {fmt(totalNonDeductibleTax)}</td>
              </tr>
            </tfoot>
          )}
        </table>
