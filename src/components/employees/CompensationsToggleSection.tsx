...
// Read-only visibility of compensations (same screen as the owner portal)
const PERMS: Array<{ feature: string; perm: string }> = [
  { feature: "compensations", perm: "view" },
];
const APP = "call_center_feedback";

export default function CompensationsToggleSection({ employeeName, authUserId }: Props) {
  ...
  const handleToggle = async (checked: boolean) => {
    ...
    try {
      ...
      toast.success(checked
        ? `تم تفعيل الاطلاع على التعويضات لـ ${employeeName}`
        : "تم إيقاف صلاحية التعويضات");
      setEnabled(checked);
    } catch (e: any) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <label className="text-xs font-medium flex items-center gap-1">
        <HandCoins className="h-3.5 w-3.5 text-violet-600" /> التعويضات
      </label>
      {saving && <Loader2 className="h-3 w-3 animate-spin text-muted-foreground" />}
      {enabled && (
        <Badge className="bg-violet-100 text-violet-700 hover:bg-violet-100 text-[10px] border-violet-200">
          مفعّل
        </Badge>
      )}
    </div>
