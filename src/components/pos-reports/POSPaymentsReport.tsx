...
const COLORS = ["#E8A020", "#F59E0B", "#10B981", "#0EA5E9"];
    ...
    <div className="space-y-4">
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        <div className="px-4 py-3 border-b border-border">
          <h3 className="text-sm font-semibold text-foreground">توزيع طرق الدفع</h3>
        </div>
        <div className="p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3 flex flex-col justify-center">
              {paymentBreakdown.map((p, i) => (
                <div key={p.method} className="flex items-center justify-between p-3 rounded-md bg-secondary border border-border">
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full" style={{ background: COLORS[i % COLORS.length] }} />
                    <span className="text-sm text-foreground">{p.method}</span>
                  </div>
                  <div className="text-left">
                    <span className="text-sm font-bold font-mono text-foreground">₪{p.amount.toLocaleString()}</span>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-secondary border-b border-border">
                ...
                <th className="text-right px-4 py-2.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">العميل</th>
            ...
            <tbody className="divide-y divide-secondary">
              {creditOrders.slice(0, 20).map(o => (
                <tr key={o.id} className="hover:bg-secondary transition-colors">
                  ...
                  <td className="px-4 py-3 text-right text-sm text-foreground">{o.customer_name}</td>
                  <td className="px-4 py-3 text-left font-mono font-bold text-sm text-foreground">₪{o.total.toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
