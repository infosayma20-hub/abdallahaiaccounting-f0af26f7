 ...
 */
export function exportMonthlyInventoryToExcel(payload: any): void {
  ...
  byCat.forEach((items, category) => {
    items.forEach((l) => {
      aoa.push([category, l.item, l.unit, Number(l.qty) || 0]);
    });
    const sub = items.reduce((s, l) => s + (Number(l.qty) || 0), 0);
    aoa.push(["", "", `إجمالي ${category}`, sub]);
  });
