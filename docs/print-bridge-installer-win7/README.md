# Unify Print Bridge — دليل التثبيت السريع

جسر الطباعة المحلي الذي يربط نقطة بيع يونيفاي بالطابعات الحرارية (شبكة أو USB).
...
- **الخدمة تعمل لكن الطابعات لا تطبع**
  افتح يونيفاي → `/onboarding/new-device` → خطوة الطابعات، عدّل الـ IPات أو استخدم زر "البحث عن طابعات الشبكة"، ثم اضغط حفظ. التغييرات تُحفظ في `C:\print-bridge\device.json` وتُطبَّق فوراً (بدون تعديل JS).
...
- لا تُغيّر API الـ bridge من جانب يونيفاي — التحديثات هنا فقط على ملفات التشغيل.